...
## §6 — Environment network-access walkthrough (docs-verified 2026-07-08, T1 code.claude.com; WEB-ONLY, not mobile app)
1. Desktop browser → **claude.ai/code**. 2. Find the **cloud icon** with the environment name (in the environment selector when starting a task, or in a routine's edit form under Instructions). 3. Click it → environment list → **hover the LLMNA environment → click the settings icon** that appears on the right. 4. In **"Update cloud environment"** → **Network access** dropdown → choose **Custom**. 5. In **Allowed domains** enter one per line: `stooq.com` / `fred.stlouisfed.org` / `api.frankfurter.app`. 6. KEEP the checkbox **"Also include default list of common package managers"** CHECKED (unchecked = only your list, would break other tooling). 7. **Save changes**. ⚠️ Applies to NEW sessions only — running/resumed sessions keep the old policy; first effect = the next wake or fresh session.
**Routine transcripts (E6 diagnosis, same visit):** **claude.ai/code/routines** → click "KR-JP morning wake" or "EOD synthesis" → the detail page lists past runs → each opens as a full session transcript. They also appear in the main session sidebar.
