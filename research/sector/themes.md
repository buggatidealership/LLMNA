...
| 3 | **POWER** — but the binding variable is **TIME-TO-POWER**, not quantity-of-MW (user reframe 2026-06-30, research-verified; = the ORIGIN worked example of `meta/time-to-x-framework.md` §Power) | Grid interconnection ~7-8yr (PJM; LBNL *Queued Up 2025*), gas turbines sold out 2029-2031 (GEV), transformers/switchgear 3-5yr (>50% of 2026 US DC builds delayed) → the constraint is ENERGIZATION LATENCY. Bypass = behind-the-meter on-site gen (skip the queue) + resilience/firming (Critical Rule #9) | **time-to-power pure-plays:** SEI (mobile turbines), BE (fuel cells, queue-skip), GEV (turbines, diluted); **resilience:** VRT/POWL/ETN/ENR (gear). NOT VST/CEG (firm but compress nothing) | **UNDERWEIGHT — the gap leg; framework = time-to-power not quantity** |
...
**2026-07-07 framework back-ref:** the application-layer framework (`sector/application-layer-framework.md`, 4-agent step-0) STRENGTHENS T5's demand driver — agent failure/rollback data (74% rolled back live agents T1 Sinch; ~95% pilots no P&L return, MIT) is exactly the axis-A spend generator, and the re-entry-relevant finding: NOW prints ~50% of new ACV non-seat + Now Assist $750M ACV (T1-class) = the usage-re-basing winner profile; DDOG 120% NRR. Both named in the framework's exit-cohort re-underwrite flag (user-gated).
