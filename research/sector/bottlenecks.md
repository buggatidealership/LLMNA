...
## [2026-07-22] B47/U8 falsifier-side ledger routing (step-4b tripwire; per `signals/cross-source-log/2026-07-22-wed-tue-catchup-14screenshot-batch.md`)
Two efficiency/content-class claims routed to the demand-destruction side of the DRAM/LPDDR row — logged so the falsifier ledger stays honest, with today's adjudication:
1. **Vera Rubin SOCAMM cut (192→96GB modules)** — verified SUPPLY-RATIONING (suppliers meet ~60% of NVDA LPDDR allocation need; TrendForce Jun-10 T2: "should not be interpreted as a reduction in Nvidia's total memory requirements"). NOT demand destruction; strengthens the LPDDR5X tightening row above. HBM row untouched (288GB/chip intact; separate 22→20 TB/s bandwidth-spec trim = per-pin-speed datum only).
2. **CMX/flash-offload-cuts-inference-DRAM hypothesis** (Bercovici relay) — UNSOURCED causal chain; CMX itself real (flash KV-cache tier, CES Jan-26). Logged falsifier-side as a WATCH: if NVDA or a hyperscaler quantifies DRAM-per-inference-unit reduction attributable to flash context tiers, the DRAM-demand row gets a genuine efficiency-compression entry. No such quantification exists today.

## [2026-07-22 EVE] Foundry-pricing receipt + FIRST dated memory crossover (per `signals/cross-source-log/2026-07-22-wed-eve-3input-tsmc-2027-hike-nand-crossover-hybrid-bonding.md`)
1. **TSMC +5-10% base from start-2027, advanced AND mature nodes; +10-15% additional ONLY on incremental HPC** (T1 Nikkei origin + native corroboration + TSMC non-denial; "up to 25%" headlines = the two stacked, not a broad hike). The mature-node inclusion = scarcity rent spreading beyond AI lanes; also a 2027 BOM-inflation input for every buyer. Cross-signal: memory pricing +~50%-class per quarter vs foundry single-digit-annual — the rent concentrates in memory, not logic.
2. **NAND row gets its first END-DATE: bit gap −4~5% through 2026 → POSITIVE 2H2027** (T1 TrendForce 20260721-13148). Mechanism is two-sided: demand-side bypass executing (smartphone production −15~20% 2026, notebooks −10% — device makers rationing away from the constrained input; TrendForce's own figures) + supply-side bypass (YMTC ramp, TC-14). Counter-bypass that pushes the date LATER: CMX-class flash-substitution-for-DRAM adds NAND demand (falsifier on 2H27). **DRAM/HBM: NO dated crossover exists** (earliest new supply mid-2027, main ramps 2028-2030 per §above) — the NAND-dated/DRAM-undated asymmetry is now an explicit register datum.
3. Falsifier-side (B47/U8 honest-ledger): the −15~20% smartphone cut IS memory-cost demand destruction at the consumer edge — logged as the demand-side counterweight to the tightness rows; if server demand ever decelerates while consumer stays cut, the crossover pulls FORWARD.

## [2026-07-27] CPO SWITCH-COMPONENT CONSTRAINT MAP — a SECOND claimant on 2.5D/3D packaging (new row, per `signals/cross-source-log/2026-07-27-mon-duv-headline-vs-body-cpo-bottleneck-micron-meta.md` §2)
Source: TrendForce, July 2026 (🟡 T2 vendor research — **qualitative only: no unit counts, no dates, no capacity figures**; that absence is why this is a constraint map and not yet a call).
| Component | Primary bottleneck | Key constraint | Near-term outlook |
|---|---|---|---|
| **Optical Engine** | integrates lasers, modulators, photodetectors, optical coupling — highly complex | high yield requirements + thermal management cap the pace of expansion | mass-production capacity limited; rapid expansion difficult |
| **SiPh** | high-precision optical coupling + advanced packaging | tight foundry AND backend packaging capacity | long build-out cycles; limited near-term flexibility |
| **Advanced Packaging** | switch ASIC + optical engines + high-speed electrical integration; **TSMC COUPE critically enabling** | **AI chips and HPC processors compete for the SAME 2.5D/3D capacity** | constrained; competition intensifying |

**The load-bearing line: CPO is not an independent supply story — it is a SECOND CLAIMANT on the CoWoS-class advanced-packaging capacity the AI-accelerator thesis already depends on.** Every CPO switch built consumes substrate an accelerator would otherwise take. This is a contention mechanism the bottleneck rows did not previously carry.
**Cross-link to TC-7 (InP geopolitical bottleneck + JP rent migration, ACTIVE) — recorded as a NOTE, not an instance count:** optical engines integrate **lasers**, i.e. the InP-substrate layer TC-7 tracks. CPO scaling is a demand multiplier on exactly that constrained substrate. First datum in this corpus connecting the CPO switch layer to the InP cluster.
**Bypass-route (Critical Rule #9, mandatory):** consensus answer to "CPO is packaging-constrained" is buy the packaging supplier. The bypass is what integrators do when CPO capacity misses their timeline — **retain pluggable optics / linear-drive (LPO) longer** rather than wait for allocation. That makes near-term CPO scarcity a **transition-DELAY** story rather than a demand-destruction story, and the loser is the CPO-exclusive supplier, not the optics chain. Named consequence: **connector/optical-module incumbents retain volume longer than a CPO-transition timeline implies.**
**Routing decision, stated so the skip is auditable (Rule #14): NOT promoted to a triangulation cluster and NOT cascaded per-name.** A qualitative table with no numbers cannot clear the bar for a thesis change, and this corpus holds no CPO-primary position. Re-eval when a unit-count or capacity figure attaches.

**B47/U8 falsifier-side ledger — MODEL-LAYER sub-leg added (2026-07-27, step-4b tripwire, per `signals/cross-source-log/2026-07-27-mon-four-ai-brief-batch-cxmt-cluster-nvda-financing.md` §6):** Moonshot AI **Kimi K3** open weights claim near-GPT-4.5 / Claude-4-Opus-class performance at **"2-3x lower inference compute"** (T3, vendor-adjacent, no independent benchmark). Routed because step 4b fires on any X-times efficiency claim regardless of verdict. **Verdict: Jevons-dominated (my model), and WEAKER in kind than the standing entries** — Frozen v2 (6-10x tokens/watt) and Etched (claimed 20x H100) are SILICON-layer claims; a 2-3x reduction from model architecture is compression-within-paradigm, the class the 2026-06-25 Jevons adjudication put under H1 (P~65%, my model) where 8-14x/yr token-volume growth dominates. **Critically it does NOT touch the memory substrate:** per the standing specialization-curve note, every point on that curve leaves KV-cache + activations HBM/DRAM-bound — a cheaper-to-serve model serves MORE tokens against the same substrate. **The channel that would matter is 2nd-order and slow:** frontier-adjacent open weights compress closed-provider PRICING, which pressures the lab-layer margin that funds capex. Logged as a slow variable on this ledger; **no bottleneck-row change.**

**B47/U8 falsifier-side ledger — AUTO sub-leg added (2026-07-24, per `signals/cross-source-log/2026-07-24-fri-ai-brief-intake-gm-memory-auto-amd-venice-3agent.md`):** GM Q2 call (T1, Jul-21): DRAM one of three buckets in a $1.5-2B FY26 cost headwind, backloaded H2 ("primarily memory" = refuted framing; tariffs $2.5-3.5B separately are bigger); BYD 天神之眼B option +21.2% (Apr-28, BYD's own DRAM/NAND attribution, T1-native, stale-recycle vs the brief). COST datum real; REALIZED demand destruction MUTED (BYD buyers "not sensitive"; GM raised FY guidance twice despite the headwind). Auto ≈ ~5% of DRAM bits (T2/T3) = squeezed price-taker, NOT a balance-tightener. Consumer-edge cluster now: smartphone −15~20% cut / Framework LPCAMM2 / Ericsson / + auto (GM/BYD). Also: Etched "20× H100" + "$1B booked orders" (vendor-claims, zero independent, Jul-23 round) = reference increment on the specialization-curve/Frozen-v2 efficiency line — every such claim stays on this ledger until independently benchmarked.
