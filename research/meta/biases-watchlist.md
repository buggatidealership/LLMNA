...
9. **2026-08-04** — 🔴 **THE SAME HEADLINE STRING AS CATCH #7, BYTE-FOR-BYTE, 56 DAYS LATER.** Operator shared 8 editions of "AI Intelligence Brief"; the 2026-08-01 AM edition carried *"Meta acquires 49% of Scale AI at ~$30B valuation"* — **an exact-string match to catch #7 above** (verified: `grep -c` returns 1 hit against this very file). Caught pre-cascade again. Verification established: the underlying SemiAnalysis piece is dated **2025-07-11**, i.e. **386 days / 12.7 months stale**; the real transaction was 2025-06-12/13, **49% non-voting for $14.3B → computed post-money $29.18B** (the "~$30B" is a 2.80% rounding inherited from the 2025 write-up, not a new mark); **no 2026 transaction exists** after 8 independent searches. Artifact: `signals/cross-source-log/2026-08-04-tue-ai-newsletter-batch-8-editions-intake-audit.md` §3.

   **Aggravating:** SemiAnalysis had published a genuinely fresh Meta piece on **2026-07-09** (23 days old, same publisher, same subject). The brief surfaced the 386-day-old one instead. And it **omitted the three real 2026 Scale developments** — deSouza named CEO effective 2026-08-10, a $500M DoD award in May, and a valuation mark frozen since June 2025.

   🔴 **THE FINDING THAT MATTERS IS OUR OWN, NOT THE AGGREGATOR'S. We had already caught this exact string and filed it here on 2026-06-06 — and neither the intake nor my own flagging retrieved it.** I pattern-matched it to B40 from memory of the *bias*, then spent an Opus verification subagent (~24k tokens, 16 tool calls, 155s) re-deriving facts that **a one-line grep of this file would have returned instantly**. The corpus knew; the retrieval didn't fire. **This is L53 (retrieval-drawer) applied to the harness's own bias ledger:** a catch that is filed but not indexed against incoming content is functionally not a catch. B40 is measured as "enforcement working" because the verification layer keeps intercepting — but the verification layer is the *expensive* backstop, and it has now been paid three times for one story.

   **Mechanically detectable, and cheap:** an exact-string / near-duplicate check of incoming headlines against the B40 catch list in this file would have flagged this in milliseconds. **Hook candidate booked** — the failure is not judgement, it is that nothing looks.

10. **2026-08-05** — 🔴 **NOT CAUGHT AT INTAKE. I INGESTED IT, WROTE IT INTO A LIVE PREDICTION, AND A COMMISSIONED VERIFIER CAUGHT IT.** The 08-04 Evening Edition carried the Kioxia+SanDisk **"332-layer BiCS10 QLC, 37.6 Gb/mm²"** joint press release as an 08-04 item. Verification established the **result was presented at ISSCC 2026, Feb 15–19** — the 08-04 release is corporate packaging of a **~5.5-month-old** technical result (with a 2026-07-03 TLC-sampling PR in between). **I had already written it into `predictions/2026-07-02-SNDK-Q4FY26-earnings-prediction.md` as "marginal positive; newness under verification"** — i.e. I flagged the doubt in words and cascaded the item anyway.

   🔴 **This breaks the "3 of 3 pre-cascade" record below, and the break is the finding.** Catch #9 was pre-cascade because the claim was *implausible on its face* (a 12-month-old M&A headline). This one was **plausible, from a real primary (both companies' newsrooms), on a genuinely live product line** — B40.1's hard case. **The tell was available and I did not use it: a technical density figure quoted to three significant figures is a conference-paper number, not a press-release number.** Filed as the discriminator for next time.

   **Aggravating and relevant to the live call:** this landed **one session before an earnings print I am graded on**, and I counted it as new information in the setup. It changes nothing directionally (it was marked "marginal"), but a re-report ingested as news inside a prediction window is exactly the mechanism by which a stale item becomes a graded error. Artifact: the CONFOUND RESOLUTION block in the SNDK prediction file.

11. **2026-08-05** — 🔴 **THE RETRIEVAL FAILURE IS NOW N=2 IN 24 HOURS, AND THIS ONE I SAID OUT LOUD TO THE OPERATOR.** He shared WSJ headlines showing SpaceX reporting quarterly results and its shares falling. I told him *"my corpus has SpaceX as private."* **The corpus knew, in two places** — `meta/private-tracker.md` carries *"SpaceX IPO June 12"* twice, and its `xAI → SpaceXAI` block contains a **2026-07-14 STATE REPAIR** headed *"house record was STALE — entity is PUBLIC,"* naming ticker **SPCX**, the $135 offer and the day-one tape.

    🔴 **A repair entry written three weeks ago, whose entire purpose was to flag this exact staleness, did not fire when the identical question arrived.** That is worse than catch #9: there the corpus merely held the fact; here it held a *correction to this specific error*, indexed under the right entity, and the retrieval still didn't happen. **The fix that was written for this failure could not prevent its own recurrence, because nothing looks.**

    **Structural cause, and it is mechanical:** a listed ~$1.3-1.65tn equity was filed in the **privates ledger** — the one file a reader never opens for a public company — while `research/companies/` held 100+ folders and not `SPCX/`. **Fixed 2026-08-05 by creating `companies/SPCX/thesis.md`.** But the generalisable point stands: **file location IS retrieval probability.** A fact in the wrong drawer is not a slow fact, it is an absent one.

    **This strengthens the hook candidate booked at catch #9** from "nice-to-have" to load-bearing: an entity-name check of incoming content against `private-tracker.md` STATE-REPAIR blocks and the `companies/` directory listing would have caught this in milliseconds. **Two misses in 24 hours on the same mechanism.**

**Post-promotion empirical pattern (updated 2026-08-05):** ~~3 of 3~~ **3 of 4** post-promotion catches intercepted PRE-CASCADE (vs the origin case, which cascaded through 5 thesis candidates first). Critical Rule #12's verification protocol is operating as designed **at the interception layer**. But catch #9 shows the layer *above* it is not: **the same claim has now been caught three times without the prior catches ever shortening the third one.** No status change to B40 itself; it remains VERIFIED-HIGH-CONFIDENCE. **Source-quality escalation adopted 2026-08-04: "AI Intelligence Brief" is QUARANTINED BY DEFAULT** — no item enters a signal cluster, a triangulation count, or a thesis file without an independently date-pinned primary. It is a surface that points at stories, not a fact layer.
...
**B64 AMENDMENT (2026-07-22) — the REVIEW-ORACLE's own lineage-independence is itself affinity-exposed:** B64 covered affinity in MY outputs; the mirror is affinity in the ORACLE I use to check my outputs. Kimi K3 (the cross-family review oracle) was accused 2026-07-22 (White House/Kratsios) of being distilled from Anthropic's Claude Fable 5 — unproven, weight-forensics pending ~Jul-27. If even partly true at the POST-TRAINING layer, K3's review judgments correlate with Claude's → the "independent blind spots" premise degrades exactly where review behavior lives. **Binding adjustment:** K3 cross-family independence HIGH→MODERATE; weight K3 by DISAGREEMENT (hole-finding preserved + reproduce-gate protects it), DISCOUNT agreement (a K3 "SAFE" is necessary-not-sufficient, ~⅓-½ discount). **The oracle's provenance is a re-verifiable VARIABLE, not a constant** — re-check at each oracle-model change, not assume-once. Full adjudication: `meta/redteam/2026-07-22-K3-independence-degrade-moonshot-distillation.md`. Re-eval ~Jul-27 (K3 weight release = T1-testable). **🔴 CORRECTION 2026-07-25 (per `meta/redteam/2026-07-25-opus5-vs-fable5-model-selection-dive.md`): "weight-forensics results expected ~Jul-27" was a CONFLATION.** No forensic study is scheduled for that date. What IS scheduled 2026-07-27 is **Kimi K3's open-weights release (2.8T params)** — which ENABLES third-party forensics but is not itself a result. **Expect raw material on 07-27, not a verdict**; the K3-independence degrade (HIGH→MODERATE, weight-by-disagreement) therefore stands UNRESOLVED past 07-27 until someone actually runs the forensics. Separately: the Kratsios allegation (2026-07-22) is CONFIRMED-AS-MADE but its SUBSTANCE is UNVERIFIED — no logs/training records published, Moonshot denies, and the strong timeline objection is that Fable 5 was offline 12 Jun–1 Jul while K3 appeared weeks after restoration.
...
### B66 — ENTITY-TYPE COLLAPSE: a source renders a named legal entity as an abstract noun (**CANDIDATE, N=1, codified 2026-08-04**)

**Origin specimen (2026-08-04).** An aggregator reported: *"**Situational awareness thesis** has declined 67% in July."* Every number in that sentence is correct. **Situational Awareness LP is a hedge fund** (CIK 0002045724), and the −67% is its month-to-date July NAV return. It became "a thesis" because Leopold Aschenbrenner's 2024 essay shares the name. The source collapsed the legal entity into the abstract noun it is named after — and then filed the item under **"Regulation & Policy"**, which contains no regulatory content whatsoever.

**Why this is a distinct failure mode and not an instance of an existing bias:**
- **Not B40 (temporal freshness):** the item was genuinely fresh — WSJ 2026-07-30/31, ~1 day old at publication.
- **Not B11 (uncited/fabricated numbers):** 67% is the published figure and survives an independent arithmetic check (+439% H1 × 0.33 = **+77.9% YTD**, against reporting of "up around 80% on the year" — reconciles to 2.1pp, which is itself what proves it is a NAV return rather than a change in portfolio market value).
- **Not B41 (signal completeness):** nothing was missing. Everything present was accurate.

**The failure is in the SUBJECT, not the predicate.** A correct number attached to the wrong *kind* of thing. And that is exactly why it is dangerous: **every spot-check of the figure passes.** The verification instinct is trained on numbers, and this failure leaves the numbers untouched.

**What it would have cost.** The harness has a 13F review of Situational Awareness LP diarised for **2026-08-14**. Read as "a thesis declined 67%," the item is market commentary and routes nowhere. Read correctly — a $45bn fund down 67% in a month, its public book sold to Citadel — it is direct evidence on the **forced-seller** question that review exists to settle. **The same sentence is either noise or the answer, depending entirely on what kind of noun the subject is.**

**Detection rule (manual until N=2):** when an item attributes a *quantity* to a *concept* — a thesis, a trend, a strategy, an idea — ask whether the concept shares its name with a fund, a company, an index, or a product. Concepts do not have returns, filings, or investors. **A number that only makes sense if the subject is an entity IS the tell**, and it is checkable in one search.

**Blind-check (#51):** distinguishes *"the subject of this sentence is the thing that has the property"* from *"the subject is a homonym of the thing that has the property"* · reads on whether the predicate (a return, a filing, a headcount) is a property a concept can even possess · **goes blind if** the entity and the concept are genuinely both plausible subjects — an index and the idea it tracks, a company and the common noun it is named after — where a category error produces no type mismatch to notice. It catches "a thesis declined 67%"; it would not catch "Palantir fell" if there were also a thing called palantir.

**Falsifier / re-eval (2026-09-04):** if 30 days produce no second instance, this is an N=1 curiosity from one low-quality aggregator and should be folded into B40's source-quality note rather than carried as its own bias. If a second instance appears **from a different source**, promote — that would make it a property of how aggregation works, not of one publisher.
