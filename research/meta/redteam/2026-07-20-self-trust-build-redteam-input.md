...
**1a-bis. Fix the grounding false-positive.** `'30.2%'` fired despite being in `day-state.md` as "+30.2% MTD". **IMPORTANT UPDATE (2026-07-20, live re-fire during the writing of this doc):** the FP is **NOT statically reproducible** — calling `is_grounded_in_repo('30.2%')` directly returns `True` (the grep finds it in `day-state.md` + others). So the miss is **fire-time state/timing-dependent**, with three converging hypotheses, all pointing at the same fix: **H1 (~35%) transient grep failure/timeout** — `is_grounded_in_repo` catches any exception and returns `False` (fail-conservative → fires); **H2 (~35%) token-normalization mismatch** — the regex `\d+(?:\.\d+)?\s*%` can capture a form whose `grep -F` needle differs from the file's exact `30.2%`; **H3 (~30%) working-tree/scan-timing state**. **Consequence: 1a (logging) is a HARD PREREQUISITE for 1a-bis — the log MUST capture, at the instant of firing, the exact `needle` string, the grep `returncode`, and any exception, or these FPs remain permanently un-debuggable.** Fixes once diagnosable: make the grep call retry-once on nonzero/exception before concluding not-grounded; normalize the needle (strip trailing `%`/`$`, compare numeric core) WITHOUT grounding genuinely-fabricated numbers by coincidence.
- *Verify:* regression test with `'30.2%'` + the file content → grounded=True; a genuinely absent number → grounded=False; a simulated grep-timeout → must NOT silently fire without logging the cause.
- **H2 CONFIRMED (2026-07-20, live + reproduced):** there are TWO distinct FP mechanisms, not one. **(A) whitespace-capture [CONFIRMED]:** `find_violations` returns `m.group(0)` from `\d+(?:\.\d+)?\s*%`, which INCLUDES the whitespace matched by `\s*`. When assistant text contains `30.2<space>%`, the needle becomes `'30.2 %'` and `grep -F '30.2 %'` fails against the file form `30.2%`. Direct test: `is_grounded_in_repo('30.2 %')` → False, `is_grounded_in_repo('30.2%')` → True. Fix: normalize the needle before grep — collapse internal whitespace between the numeric core and the unit (`re.sub(r'\s+','',needle)`), ground if EITHER the raw or collapsed form matches (safe: `30.2 %` and `30.2%` are the same numeric claim). **(B) no-space variant [STILL OPEN]:** the ORIGINAL fires earlier the same day reported a no-space `'30.2%'` token that IS statically grounded — mechanism unconfirmed (H1 transient grep timeout/exception fail-conservative, or H3 scan-timing). (B) is un-diagnosable without the 1a fire-time log capturing needle + returncode + exception — reinforcing that 1a precedes 1a-bis.
