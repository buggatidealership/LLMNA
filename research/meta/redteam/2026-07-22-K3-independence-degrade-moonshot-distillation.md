...
## ADDENDUM 2026-07-23 — US state-attribution layer (agent-verified; posture UNCHANGED)
WH OSTP Director **Michael Kratsios** (X post, Jul-22) put the distillation accusation on government record ("We have information that Moonshot AI distilled Anthropic's Fable for the development of its K3 model") + NEW single-source layer: Moonshot "accessed GB300s in Thailand, likely to train" (his own hedge "likely"; no procurement route disclosed). **Evidence floor UNMOVED: no forensics/indictment/intel released — multiple outlets explicit; the core charge is Anthropic's Feb-2026 accusation re-asserted.** Bessent sanctions/Entity-List threat = rhetoric, nothing enacted. Moonshot DENIED (KDA/AttnRes original-architecture claim). Expert pushback: ~15-day Fable-5→K3 gap argued infeasible for FULL distillation (targeted post-training possible-unproven) — marginally cuts AGAINST the maximal claim. **Jul-27 open-weights release confirmed as forensics catalyst; early partial exists (Greenblatt/Redwood cross-entropy: K3 self-identifies as "Claude" — circumstantial).** DECISION: the MODERATE agreement-discount on K3-as-reviewer holds AS-IS — political escalation without evidence neither tightens nor relaxes it; re-adjudicate on the Jul-27+ forensics as pre-registered. Watch: if sanctions/Entity-List ENACT, K3 access itself may become a compliance question — flag for the operator before any further K3 commissioning if that fires.
