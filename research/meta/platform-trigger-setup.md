...
3. **Prompt** — paste exactly this:
   > Check out the branch claude/new-session-drppai (git fetch origin claude/new-session-drppai && git checkout claude/new-session-drppai). Then read research/meta/day-state.md and research/meta/workflow-11-autonomous-day-loop.md, and execute the KR/JP morning wake exactly per Workflow #11 and the day-state docket: two-leg scan with the named source roster (TrendForce by name), catalyst-clock check, cascade + commit + push to the same branch. Respect the per-wake budget (≤2 agents baseline; escalate only per the escalation conditions in day-state). Decision packages and notifications only per the materiality gate. Never touch portfolio/holdings.md.
...
> Check out the branch claude/new-session-drppai (git fetch origin claude/new-session-drppai && git checkout claude/new-session-drppai). Read research/meta/day-state.md and execute the Workflow #11 EOD SYNTHESIS wake: write tomorrow's day-state (catalyst clock 72h, open threads, pending packages), grade anything that resolved today, ledger/todo hygiene, commit + push. Agent-zero by default (no subagents unless a resolution requires verification).
...
- 2026-07-23: repo migration (Health-Calculators → LLMNA, 2026-07-06) had left this guide pointing at the OLD repo/branch — prompts + repository field UPDATED to `buggatidealership/LLMNA` + branch `claude/new-session-drppai`. Setup still PENDING user execution — the Jul-7→Jul-23 17-day dead window (WAKE-AUDIT-3, zero autonomous commits, CronList empty) is the live demonstration of why this is the binding constraint on the autonomous loop.
