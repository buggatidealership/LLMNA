...
# Commits-behind-origin/main at which the session-start banner escalates from a
# one-line note to a full stop-work alarm. Set 2026-07-25: the observed failure
# was 668 behind; ordinary same-day drift is 1-5. 25 sits well clear of both, so
# the siren stays rare enough to still mean something when it fires.
STALE_ALARM_THRESHOLD = 25
...
def branch_position() -> list:
    """Is this session looking at the LIVE system, or a stale branch?

    Codified 2026-07-25 after FIVE consecutive misdiagnoses in one session
    ...
    No network, no timeout risk. A fresh clone's origin/main is accurate, which
    is exactly the case that failed.

    FALSIFIER (re-eval 2026-08-25): if the 🛑 alarm fires in a session that then
    proceeds to audit/grade WITHOUT merging, the banner is decorative and the
    check must move from advisory to enforcing (a Stop-hook block on audit-class
    output from a stale tree). If it never fires in 30 days, confirm the cause is
    that sessions now land live -- not that the ref comparison silently broke;
    prove it by asserting the LIVE line is present, not merely that no alarm ran.
    """
    import subprocess
    ...
    if not counts:
        return []  # not a repo / no origin/main -- say nothing rather than guess
    try:
    ...
    out = []
    if behind == 0:
        out.append(f"BRANCH: {branch} — up to date with origin/main "
                   f"({ahead} ahead). Corpus below is LIVE.")
    elif behind < STALE_ALARM_THRESHOLD:
        # Routine drift. Note it, do not alarm -- an alarm that fires on every
        # ordinary session is the macro-anchor pathology (152 fires, one reason,
        # ~5/day, no longer carrying information). Keep the siren scarce.
        out.append(f"BRANCH: {branch} — {behind} behind / {ahead} ahead of "
                   f"origin/main. Routine drift; pull before auditing.")
    else:
        out.append("🛑 STALE BRANCH — YOU ARE NOT LOOKING AT THE LIVE SYSTEM 🛑")
        out.append(f"  {branch} is {behind} commit(s) BEHIND origin/main "
                   f"({ahead} ahead).")
        out.append("  Do NOT audit, grade, retire, or diagnose from this tree —")
        out.append("  findings measured here have been WRONG every time (2026-07-25, n=5).")
        out.append("  Merge or rebase origin/main FIRST, then re-measure:")
        out.append("      git fetch origin main && git merge origin/main")
    out.append("")
    return out
