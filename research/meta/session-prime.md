...
**The file sat 347 chars over `MAX_INJECT_CHARS=30000`, so everything past char 30,000 was silently dropped at every cold-start injection** — in the file, in git, readable, and never reaching the session it exists to prime. **The failure mode is the cap's, not the content's:** it truncates instead of failing, 1 char over and 5,000 over behave identically, and the symptom is content that quietly stops arriving. Fixed by pruning, not by raising the cap.

**Blind-check (#51):** distinguishes *"session-prime is priming the session"* from *"its tail is being cut off"* · reads on `len(file)` vs `MAX_INJECT_CHARS` · **goes blind if** the cap is enforced somewhere the file's authors never look. **A limit checked only by the thing consuming it is invisible to the thing producing it.**

**RESOLVED:** the booked pre-commit length assertion shipped and has since blocked two real over-cap commits (08-06, 08-07). ⚠️ **Measure in CHARACTERS, not bytes** — this file carries Korean text and arrows, so `wc -c` overstates by ~370 and will tell you you are over when you are not (mis-read once, 08-07).
...
- **L60 CANDIDATE (2026-08-07): WHEN AN EXTERNAL ATTRIBUTION ARRIVES, CHECK WHETHER THE CORPUS ALREADY DERIVED A BETTER ONE.** On 08-06 I wrote, from our own tape, that SK Hynix fell on the **SanDisk read-through** — correct. On 08-07 I silently replaced it with a Korean-press *"next-gen AI chip supply-schedule delay"* and called it hard evidence for the operator's fab-gated thesis; a verifier **refuted every leg** (no lineage in six 08-06 articles; the "SOX crash" was **−1.40% after +6.55%**; true driver = the SNDK/WDC guidance miss I had already named). **Displacement leaves no trace — nothing contradicts, one story just takes the slot.** Binding: before an external attribution enters an artifact, grep for our own prior read and state *none exists* / *agrees* / *CONFLICTS and why theirs wins*. **I wrote the guard against this bias in the same section where I committed it — naming a bias is not a countermeasure.** Companion: **an inherited magnitude adjective (crash/plunge/rip) is unusable until a number is attached** (3rd instance in 4 days).
- **L59 CANDIDATE (2026-08-06): A CORRECT CALL FROM A REFUTED MECHANISM IS A REASONING MISS.** SNDK R-4 (beat-both → negative close) HIT at **P=0.35** — but its registered justification was *"consensus sits above the guide, so an in-range print is a miss."* **The print was NOT in range**: it cleared consensus by 8–15% and guided +17.7% QoQ, and fell anyway. **Premise false, conclusion landed.** Score outcome and mechanism SEPARATELY; **never update the P on a mechanism the event refuted** — a long-odds hit strengthens a disproved chain *harder* than an ordinary one. Binding: **no direction leg ships without a stated mechanism.** Companion killed same session at N=2: SNDK R-2 over-called magnitude (−8.00%), DDOG R-5 under-called it (−19.03%), identical P and threshold ⇒ **magnitude is NOISY, not biased** — keep separate from B45 (directional) or a correction factor lands on noise.
- **L58 (2026-08-05): BASIS MISMATCH — every number carries what it was measured AGAINST, and no two numbers may be compared until their bases match.** Basis = reference point, date, category, season, window, denominator — **and tense**. Generalises and SUPERSEDES L42-b. Enforcement is **CONSTRUCTIVE** (a BASIS column in every comparison table), not filtering. **Now 7 specimens in 6 days** — AMD/SPCX/SNDK headline inheritance, "printed" of an event 2.9h away, the SNDK R-2 grade, and (08-07) **the KR-OPEN WAKE reporting a 24-min open in the register of a 390-min session**: Samsung read −0.61%, closed **−6.30% (10.3×)**. Every prior wake carries it. TENSE is a basis.
- **L57 (2026-08-05): A BEAT-HISTORY BASE RATE MEASURES THE GUIDANCE GAP, NOT THE COMPANY.** A beat streak is a fact about how conservatively management guided, not about the business. **Check the gap is still open before using the streak** — DDOG's bar-jump was the company removing its own sandbag. ⚠️ **WEAKENED 08-06**: O-3, registered deliberately against my own point estimate, MISSED.
- **L53 CANDIDATE (N=2 → N=3, MPWR + MURATA 07-31; RECURRED 2026-08-06): THE RETRIEVAL-DRAWER ERROR** — a name filed under the wrong theme is functionally invisible even when it is in the corpus and correctly described. Recurred 08-06: re-derived beat-and-fall as new when `predictions/2026-08-05-DDOG-...md:301` already had it, in a file I wrote 18h earlier. Governs DECAY of a correct OLD classification (B60 = framing of NEW data). **Same family as L60 above — the corpus holds the answer and I look past it.**

_Numbering: L17/L18 tombstoned; full list `predictions/lessons.md` + `meta/tags.md`._
_§3 curation (all canonical in `predictions/lessons.md`): 07-31 L27+L28 · 08-02 L40 · 08-06 L54+L55 · **08-07 L56 rotated out for L60**; L53 now inside the rolling-5, no exception needed._
