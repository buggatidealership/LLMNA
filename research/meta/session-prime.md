...
**RESOLVED:** the booked pre-commit length assertion shipped and has blocked two over-cap commits (08-06, 08-07). ⚠️ **Measure in CHARACTERS, not bytes** — this file carries Korean text and arrows, so `wc -c` overstates by ~370 and reads OVER when the file is under (mis-read once, 08-07).
...
- **L57 (2026-08-05): A BEAT-HISTORY BASE RATE MEASURES THE GUIDANCE GAP, NOT THE COMPANY.** A beat streak is a fact about how conservatively management guided, not about the business. **Check the gap is still open first** — DDOG's bar-jump was management removing its own sandbag. ⚠️ **WEAKENED 08-06**: O-3, registered against my own point, MISSED. 🔴 **Was ORPHANED — cited here + injected into every session while ABSENT from lessons.md; canonical entry written 08-07.**
...
_§3 curation (canonical: `predictions/lessons.md`): removed 07-31 L27/L28 · 08-02 L40 · 08-06 L54/L55 · 08-07 L56 (for L60)._
