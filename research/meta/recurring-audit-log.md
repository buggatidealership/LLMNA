...
## [2026-07-21 TUE] GRADED-CALL VOLUME QUOTA weekly check #2 (formal, due today; computed)
Window 07-14→07-21 (python over grading-log + git): grading-log date-mentions 07-15 ×6 / 07-16 ×10 / 07-17 ×2 (TSMC/ASML grade cluster); 9 prediction-layer files touched; formal NEW registrations = 5 (five-calls set 07-17). Directional provenance-tagged wake-calls this week: five-calls re-weights ×3 (07-19 SUN, 07-20 EVE resolution, 07-21 re-weight 75/10/15), KR escorted readings ×2, H3 dashboard reads ×2, capex-marker #4, LTA frame-verdict (~85%→~95% closed). **Verdict: BEHIND on strict formal-call pace (~5-8/wk vs ~14 target) for the second consecutive week; the amendment's fix (formalize wake-calls as registered calls with P-provenance rows) remains the gap — routed to the 07-24 audit as a process decision (either wake-calls get grading-log stub rows at booking time, or the 150 target gets re-based honestly). Wed's triple (IBM+NOW+NBIS T+30) adds ≥3 formal events.**

## 2026-07-23 — P0 harness hardening (07-24 docket run early per NO-DELAY; dedicated Fable-5 instance, operator-funded)
Reproduce-before-fix throughout; artifact `meta/redteam/2026-07-23-fable5-harness-audit-P0-hardening.md`; commits 4b26e72 + e477487.
- 8 probe-verified git-guard bypasses fixed (commit -n after-message class ×4; rm tilde/$HOME boundary class ×4); 0 new FPs across 14 legit-command controls; bash -c scan invariant pinned.
- structural-output PI tier gate: IGNORECASE + 6 casing fixtures; marker list widened to house-standard forms (hyphenated joint-state, H-label inline P) — case-4 born-red root-caused to the hook, not the fixture.
- G-28: metric numerator reclassified (markers-missing only) — contaminated read 0.131→0.217 RISING vs clean 0.120→0.105 FALLING; the 08-06 priming-hook adjudication was about to be decided by contamination. e04eaef ref order restored to design-130 (origin/main first).
- Redirect over-block FP (16 blocks 07-19..22, incl. both 07-22 commit-message blocks) killed via data-span stripping; 3 true-positive shapes verified still blocking; fixtures pinned.
- Probe-pollution channel closed (LLMNA_PROBE=1 tagging in hooks + all 3 test harnesses + metric exclusion) after demonstrating it live (~100 untagged probe lines from checkpoint-1 runs).
- Suites: adjacency 12/12, bypass 17/17→21/21, tier-gate 10/10, scratchpad probe 31/31. NEW finding: test_framework_codifications 198/200 pre-existing RED (stale todo.md coupling) → routed to 07-24 remaining scope.

## 2026-07-24 — Monthly audit build (scheduled wake; artifact `meta/redteam/2026-07-24-monthly-audit-build.md`)
Two DUE items + four same-audit measurements. Governance: B47 hook = LIVE-enforcement → spec-then-review, NOT shipped (Rule #19; both instances externally caught = author-blind).
- **Fire-logging HOUSE STANDARD ✅ (3805eb9):** shared hook_fire_log.py (house format, fail-open, sanitize, probe-aware, selftest/clock/rotate CLI); wired the 11 silent Stop hooks; metric-safe rotation (append-only through 08-06) + archive-aware metric; clock cross-check first reading OK (~6h). Zero enforcement drift (framework 198/200 same known-RED; tier-gate + git-guard suites ALL PASS). 7 already-logging hooks NOT re-pointed (net-negative author-blind risk, documented).
- **B47 efficiency-claim hook ✅ DECIDED — SPEC only, review-gated** (`meta/hooks/b47-efficiency-claim-hook-SPEC.md`): I2 ("6-10x tokens per unit power") has no word "efficiency" = the author-blind trap in concrete form; §6 Q4 openly doubts a text-scanning Stop hook can catch a MISSING-LEG failure. Interim step-4b LIVE.
- **B47 promotion (N=2) ✅ CONFIRMED-SPLIT:** blind-spot real at N=2; self-detection still external (anti-laundering) → next instance must be self/hook-caught or the mitigation is theater.
- **B65 canary ✅ GREEN** (seeded 73,914 wafers / $84,617,203 in scratchpad → anti-fab exit=2 caught both; first exit=0 was a <200-char test-construction error, corrected+logged). **Avoidance audit PARTIAL-CLEAN** (no un-computed meta-state count claims in the commit/artifact sample; full message stream ephemeral = partial).
- **Verdict-flip sampling ✅:** 7/8 pending numbers now grounded (block drove citation = hook working); structural finding — cross-session transcripts ephemeral → historical pending verdicts are post-hoc-groundable only, not transcript-adjudicable.
- **Curation-ratio first reading ✅ (K3 dissent #1):** 56.5% maintenance vs research (telemetry-excluded, 871 substantive commits/30d); elevated (production-bias concern) but window includes the heavy 07-20→24 harness push. Falsifier: sustained >60% MoM.
- RESIDUAL carried: prose-deadline extractor wiring + parser tests; B47 hook post-review ship; full pruning pass (only the curation reading ran this wake).

## [2026-07-27 MON] GRADED-CALL VOLUME QUOTA weekly check #3 (formal, due today; computed) + folded weekly cost-yield (G-09)
Window 07-21->07-27 (python over git + grading-log): 169 commits; 8 prediction-layer files touched; **formal NEW registrations = 3** (2026-07-22 IBM/NOW/GOOGL twin-print, 2026-07-22 K3-authored print-grading rubric, 2026-07-25 SUMCO Q2CY2026 component-level); grading-log date-mentions in window 07-22 x6 / 07-23 x1 / 07-25 x3; 3 GRADED lines added; **7 dated lessons.md entries** added (L42, L43, L44, L44 promotion, plus prior-lesson promotions). Twin-print grade closed **11 legs** on 07-25 (Brier 0.1018 / 0.2812 / 0.1344 — all three independently reproduced by the fresh-session audit the same week, which is the first external confirmation of a house Brier computation).
**Verdict: BEHIND strict formal-call pace for the THIRD consecutive week.** On a LEG basis the week is materially better than check #2's 5-8 (11 graded legs + 3 registrations spanning multiple components); on a FORMAL-REGISTRATION basis it is 3. The two counts diverge because directional wake-calls never become gradeable rows.
**The finding that matters is not the count — it is that the fix was never executed.** Check #2 (07-21) routed the process decision to the 07-24 monthly audit: *either wake-calls get grading-log stub rows at booking time, or the 150 target gets re-based honestly.* Grepped `meta/redteam/2026-07-24-monthly-audit-build.md` for stub-row / re-base / wake-call decisions: **zero hits.** The decision was routed and then not made, and check #3 has now measured the same unfixed gap a third time. A measurement repeated three times without the routed fix is an instrument reporting on a process failure, not on volume.
**Action: the decision cannot be routed a fourth time.** Carried to 07-28 as a forced binary. If neither branch is chosen by check #4, the honest read is that the 150 target was never operationalised and the quota instrument should be retired rather than re-measured.
**Folded cost-yield reading (G-09, first fold):** 5 subagent fires past 7 days — 3 HIGH / 1 MEDIUM / 0 LOW / 0 ZERO / 0 FRAMING-ERROR-CAUGHT. Rule #16 falsifier threshold (>=3 ZERO) not breached. 30-day estimated spend ~2,978k tokens with the standing budget warning active. Note for the next fold: today's 2-agent INGEST fan-out produced one HIGH (leg 1: LOI non-binding + two-entity + 06-08 escalation + SEC 6-K walk-back) and one that **refuted the operator's own premise** (leg 2: ATH inverted) — the second is the highest-value class the ledger tracks and argues against trimming fan-out on cost grounds.

## [2026-08-04 TUE] GRADED-CALL VOLUME QUOTA — FORCED BINARY RESOLVED (operator decision; branch (a))

**The binary carried from check #3 was put to the operator on 2026-08-04 and decided: branch (a) — fix the plumbing, not the target.** Branch (b) (re-base 150→run-rate) rejected; retirement not reached.

**The diagnosis that decided it.** The item was framed as a volume shortfall for three consecutive weeks. It is not one. Dated directional calls are made in nearly every wake — the KR ETF-divergence test, the H1/H2/H3 reweights, the EODHD vendor-defect call — and none were written anywhere gradeable, so none could count. The FORMAL-REGISTRATION and LEG counts diverged for exactly this reason, which check #3 had already observed without drawing the conclusion. **Re-basing the target would have produced an honest-looking number out of a process nobody had measured.**

**Shipped same day, not routed:** `predictions/grading-log.md` § "Wake-call registrations `[WAKE]`" — the convention plus its first two registrations. Rows are booked AT BOOKING TIME with P-provenance, and are tagged so the quota can be read with and without them.

**The first registration already earned the convention.** The 08-04 KR ETF-divergence call went in with **`⚠️ NO P STATED AT BOOKING`** — the wake artifact hedged in prose (*"equally consistent with plain beta"*) and never wrote a number. That was recorded as-is rather than back-filled. **An unfalsifiable hedge reads as caution and grades as nothing**, and until there was a row to put it in, there was nowhere for that fact to become visible.

**What check #4 and after must measure — this is a change of instrument, not of threshold.** Not the gap. Compliance: wakes that stated a dated call ÷ wakes that booked a stub row, target ≥80% by 2026-08-24, with the `[WAKE]`-excluded registration count reported alongside every total. **If the ratio is high while the excluded count stays flat, the plumbing got fixed and nothing else did — that is a legitimate finding and the tag exists so it cannot be hidden by a rising headline number.**

**Honest limit:** this was measured three times and left unfixed, and it was an operator question — not check #4, not the instrument — that forced the decision. The instrument correctly reported a real gap every week for three weeks and had no mechanism whatsoever for escalating a finding nobody acted on. That defect is not fixed by this decision.
