...
- [ ] **P0 / harness / 2026-07-24** [INFRA, CAL, DUE] — DEEP-DIVE DEFERRALS + **K3 CROSS-FAMILY REVIEW REWORK (2026-07-22, ALL flagship claims independently reproduced — `meta/redteam/2026-07-22-K3-deep-dive-review-adjudication.md`)**. **✅ ACCOUNTING-LAYER SUBSET DONE 2026-07-22** (fresh session, branch `claude/harness-accounting-audit-it2e0w`, adjudication `meta/redteam/2026-07-22-accounting-layer-repair-adjudication.md`, each fix commit carries its receipt): (a)✅ commit `-n` anywhere-after-commit + `-C <path>` [0b214c1]; (b)✅ rm tilde/HOME/trailing-slash boundary [0b214c1]; (c)✅ PI matcher re.IGNORECASE + 6 casing fixtures [0b214c1]; (d)✅ tier-gate fixture CLAUDE_PROJECT_DIR-relative + case-4 adjudicated (hook over-fired on hyphenated joint-state) [0b214c1]; BLOCKING(1)✅ G-28 numerator reclassification + probe-tag exclusion [a16e036]; BLOCKING(2)✅ e04eaef→origin/main-first per design-130 + denominator telemetry-exclusion [a16e036]; (4)✅ 8-exit-path inventory `meta/guard-exit-path-inventory.md` [832b8ab]; (5-redirect)✅ redirect over-block FP fixed + fixtures [832b8ab]; (6-nap)✅ post-block nap window closed for receipts-hook via content-scoped guard [832b8ab]. **PLUS** receipts-hook Phase 1 [817ac3a], promise-heartbeat + registry [803547c], computed-counts generator [7aac61e]. **STILL OPEN (not this session's scope):** (3) G-19 changeset-window real hole; (5-other) exit-1-outside-repo FP; (6-other) stdin-schema-drift class + 337-agent journal receipt gap + content-scoped-guard propagation to the other 15 hooks (registered promise 08-01); (7) ~150 MEDIUM/LOW from the extract; **EXTERNAL REVIEW of this session's solo-built diff (registered promise 07-24 — the theme-2 settle gate).** **CERTIFIED (no rework):** f19663d session-start 7→83 (K3 clean on real corpus).
