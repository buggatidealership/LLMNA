...
## Archive` so future Claude doesn't redo them.
...
- [ ] **P1 / harness / 2026-08-07** [INFRA, CAL, OPT] — **FRESH-SESSION AUDIT ON A FIXED WEEKLY SLOT — the mechanism is built and has been parked 10 days** ⏸️ **OPERATOR CALL PENDING**
  ...
  - **The load-bearing reason this matters more than it looks:** a fresh session is the **falsifier for the whole external-memory premise**. What I hold that a fresh session does not is *unwritten* context, which should not exist in this system. **Nothing currently tests that, and the test is cheap.**
  - Blocker: transport requires the operator to open a session and paste a block. That step has no schedule attached — which is the entire bottleneck.
  - 🔴 **PREREQUISITE DISCOVERED 2026-08-07, and it silently breaks every fresh session until fixed: `main` is 29 commits behind `claude/first-test-new-repo-wxedu9`, spanning 08-05 → 08-07.** A fresh clone lands on `main` by default and would boot **missing L58/L59/L60, three days of day-state, and — worst — the CURRENT `session-prime.md`, so the cold-start hook would force-inject STALE LESSONS.** The session would look fine, sound confident, and be running on Tuesday's calibration. **Every fresh-session audit run before this is fixed is measuring the wrong corpus.**
  - **Two fixes: (a) name the branch explicitly in every commission/boot prompt — done for the 08-07 handoff; (b) MERGE THE BRANCH TO `main` so fresh sessions boot correctly by default — offered to the operator 2026-08-07, awaiting his call, NOT executed (branch discipline: no pushes outside the designated branch without explicit permission).**
  - **WHY main went stale — no malfunction, two safety rules interacting.** The platform assigns a named feature branch and forbids pushing elsewhere without explicit permission; separately, no PR is opened unless the operator asks. Nobody asked ⇒ nothing merged ⇒ work accumulated. `main` last moved **2026-08-05 15:09** (`0b09cdc`). **Net effect: the canonical branch stopped being canonical, and a fresh clone lands on the stale one.**
  - **Operator's position (2026-08-07): everything should be on `main`.** Concurred — branches earn their keep under code review / CI / multiple contributors, and this repo has none of those; the corpus IS the product, there is no release separate from the writing. **Recommended: merge, then commit directly to `main`, and delete the stale audit branch.**
  - **BRANCH INVENTORY (computed 2026-08-07):** `first-test-new-repo-wxedu9` **30 ahead**, last 08-07 — live corpus · `verify-investment-os-claims-5u2qb7` 27 ahead, last 07-28 — 🟢 **VERIFIED NOT LOST: zero files missing locally, F1-dossier/audit content already present in-tree (`meta/redteam/2026-07-27-independent-audit-part1-part2.md`), diff vs my branch is 766 insertions / 13,311 deletions i.e. it is mostly MISSING my Aug 5-7 work. Stale history artifact, safe to delete** · ⚠️ `harness-accounting-audit-it2e0w` **1,436 ahead**, last 07-22 — **DO NOT MERGE**, that count implies a different history root (probably the pre-migration Health-Calculators lineage); investigate before anyone touches it.
  - **Boot verified as automatic (computed 2026-08-07, not recalled):** `.claude/settings.json` is committed and registers **3 SessionStart hooks** (git + session-start-hook + session-prime-hook), **1 UserPromptSubmit** priming hook, and **17 Stop** hooks — all `$CLAUDE_PROJECT_DIR`-relative, so they ship with a clone and fire from turn 1. **⇒ A boot prompt should NOT list files to read; naming two files would narrow the session BELOW what the hooks already inject.** The only things a boot prompt must carry are the BRANCH and a cheap receipt (branch / latest day-state date / highest lesson number) to prove the boot landed before any analysis is spent on it.
  - Linked: `meta/redteam/2026-08-05-K3-commission-postcondition-audit.md` · `meta/redteam/2026-08-01-instrument-validity-audit-commission-prompt.md` (dated 08-08) · `meta/day-state.md` 2026-08-07 operator-session entry
- [ ] **P1 / harness / 2026-08-07** [OPT, INDP] — **BARE-OPUS CONTROL ARM (H-B) — the test that separates "the harness did it" from "the model did it"** ⏸️ **OFFERED, AWAITING OPERATOR GO**
  - Origin: operator proposed that contextualisation (pricing SNDK as the 4th instance of the MPWR/Kioxia/AMD pattern rather than as a standalone company) was the edge over GPT-5.6 and Kimi K3. **Likely correct — but `meta/cross-model-benchmark-protocol.md:27` already registered the caution: a bare session could have FOUND those instances and had no reason to WEIGHT them, because they were not its own graded errors. That is a MEMORY effect, not a reasoning effect.**
  - Scope: ask **bare Opus, no harness, no corpus** the same SNDK question as registered. **If bare Opus also says DOWN, the harness contributed nothing on this call and the model did it.** ~15 minutes. Pre-register the read of both outcomes BEFORE running so neither result can be narrated favourably after the fact.
  - ⚠️ Note against myself: this is the test whose most likely outcome is unflattering, and it has been the "cheapest high-value item" on this list for two days without being run.
  - Linked: `meta/cross-model-benchmark-protocol.md` (H-B row)
...
- [x] **P1 / USER-ACTION / 2026-07-13 → CLOSED 2026-08-04 (SUPERSEDED BY STANDING DIRECTIVE, operator-authorized)** — DeGiro/N26 availability check on REIA-surfaced names.
  ...
  - **The failure class is L53 (retrieval-drawer), applied to a directive rather than a name:** the superseding line lives in `CLAUDE.md` and was read at every session start; it simply never got connected to the item being written. A rule that is read but not indexed against new work is functionally not in force. **Neither the write nor the 22 days of re-reads caught it — an operator question did.**
  - **Names are NOT lost.** Moved to `watchlist/candidates.md` as the "REIA power / electrical-infrastructure cluster", ranked on merit with the exchange tag attached as a courtesy, per the 07-05 directive. Tradability gets checked by the operator if and when one earns an entry package — not before.
  - Retained factual result: Nanya 2408.TW NOT available on DeGiro or N26 (user-verified 07-13) → watchlist-INSTRUMENT only. That datum survives in `candidates.md`.
  - Linked: `signals/cross-source-log/2026-06-27-REIA-batch-power-cpuserver-cxmt-investable-list.md`, `watchlist/candidates.md`, `research/CLAUDE.md:42`
- [ ] **P2 / research / 2026-06-25→2026-07-04** [INDP, AF, BOT] — EU-sovereign-AI investable expressions: PARTIALLY CLOSED (per `signals/cross-source-log/2026-07-04-all-regions-weekend-consolidation.md`): OVHcloud = sole direct pure-play (small/risky); Schneider+Legrand = agnostic picks-and-shovels (>20% DC revenue); no sovereign DC REIT exists; financing = same US private-credit sponsors (node scope note cascaded). REMAINING: watch the EuroHPC gigafactory CALL ("summer 2026") — award winners = the next expression surface; deep-dig Schneider/Legrand DC-segment momentum before any tiering. **2026-08-04: both USER-VERIFIED tradable on DeGiro/N26 (with the whole REIA power cluster + Lenovo/Wiwynn/Tokyo Electron) — so the deep-dig is no longer speculative work against a possibly-unexecutable name. Access is NOT a promotion (07-05 directive); B39 asymmetry is still unrun on both.**
...
- [ ] **P2 / harness / 2026-08-11** [INFRA, CAL] — **EOD ROUTINE CONDITION CHECK IS TIME-BOUNDARY-BLIND — make the window RELATIVE, not absolute.** Found 2026-08-04 when the check ran and returned **0 commits since 16:00Z** on an evening carrying four substantive commits at **15:35 / 15:44 / 15:49 / 15:52 UTC — 25, 16, 11 and 8 minutes under the wire** (computed). Those commits held the operator's 8-edition newsletter batch, six Opus verifications, a thesis cascade to a name printing the next day, two corpus corrections and a new bias — i.e. **exactly the content the DEGRADE clause enumerates.** The literal verdict was "quiet evening, run the full discovery leg."
  - Scope: change the Routine prompt from `git log --since="16:00"` to **"any non-telemetry commit since the previous EOD fire"** — a window anchored to the last fire, not to an arbitrary wall-clock line. Requires an edit in the operator's Routines UI; pairs with the already-due KR-OPEN Routine text amendment (same UI, same trip).
  - **Blind-check (#51):** distinguishes "the evening was quiet" from "the evening's work landed just before the boundary" · reads on commit count after a FIXED clock time · **goes blind if** work concludes shortly before 16:00Z — which is the NORMAL shape of an afternoon session that finishes its commits before the Routine fires. **The check does not measure activity; it measures activity-after-an-arbitrary-line.**
  - Interim: I degraded on intent rather than on the check, and said so in the artifact. That works while a session is reading the Routine and thinking about it; it does not work unattended, which is the case the Routine exists for.
  - Linked: `meta/day-state.md` (2026-08-04 EOD entry), the KR-OPEN Routine amendment item

- [ ] **P2 / prediction / 2026-08-06** [CAL, INDP] — 🔴 **ETF-DIVERGENCE TEST: RE-SPEC OR RETIRE — do NOT run a 4th reading of the current design.** Resolved 2026-08-05 (FSC rules effective) and **could not adjudicate**: spread reads **−2.57pp** on the 2-vs-2 basket and **+2.75pp** against the same single control used on 08-03/08-04. **The sign flips on one name** (SEMCO +14.09%, cause unverified). Series to date: −2.09 / +2.09 / ±2.6.
  - **Diagnosis: the instrument's resolution is smaller than its noise.** The effect sought is ~2pp; B45 records ±5-12% single-day moves in this cohort as routine. **That was computable at registration from a base rate already on file.** Root cause is deeper: this is a FLOW hypothesis being measured with a PRICE instrument, because all four designated flow discriminators (투자자별, 반대매매, futures basis, CME/EUREX gap) are machine-unreachable.
  - Scope: EITHER pre-register a control basket wide enough to survive one outlier (membership fixed in advance, sized against measured cohort dispersion), OR **retire the hypothesis as unmeasurable with available instruments** and say so. Both are acceptable; a fourth reading of the current design is not.
  - Linked: `signals/cross-source-log/2026-08-05-wed-kr-open-wake-etf-test-resolves-on-one-name.md` §2, `predictions/grading-log.md` (the `[WAKE]` stub), lessons L54

- [ ] **P1 / harness / 2026-08-11** [INFRA, DUE] — **H3 — JPY AND BRENT ARE BLIND; THE RATES LEGS ARE NOT. 🟡 RE-SCOPED 2026-08-05.** The 08-04 claim ("5 of 6 ≥4d stale, median 4d, blind by construction") was measured Tuesday-against-Friday and **overstated the structural component**. Wednesday reading 08-05: **median lag 2d, only 2 of 6 ≥4d** — 10Y/2Y/breakeven/OAS all run 1-2d mid-week and are usable. **The genuine structural gaps are JPY (5d) and Brent (9d, and spot-FOB not a settle).** Fix those two; the daily check does NOT need demoting. Original overstated text: Computed 2026-08-04: five of the six H3 instruments carry a ≥4-day publication lag (10Y/2Y/JPY 4d, HY OAS 5d, Brent spot 8d; only the 10y breakeven is 1d). The KR-OPEN WAKE runs DAILY and its H3 two-path check is specified against this set, so **it cannot detect an escalation inside the window it exists to monitor.** The 2026-08-03 "10Y flat, no escalation" error was this set behaving as designed, not carelessness.
  - Two acceptable fixes, one unacceptable: (a) add at least one same-day route (futures, live quote) so the daily check can actually see today; (b) **demote H3 from a daily leg to weekly and say so**, which costs nothing since the instruments only update weekly anyway. NOT acceptable: leaving it daily and continuing to report "unchanged" when the honest statement is "invisible."
  - Linked: `signals/cross-source-log/2026-08-04-tue-kr-open-wake-vendor-sign-inversion-h3-lag.md` §3, `meta/data-access.md`
...
- [x] **P1 / process / 2026-07-02 → CLOSED 2026-08-04 (SUPERSEDED, operator-authorized)** — WORKFLOW #11 AUTONOMOUS DAY-LOOP. The 5 session-scoped wakes armed 2026-07-02 carried a 7-day expiry and **lapsed ~2026-07-09; they were never re-armed.** Its own 30-day falsifier audit fell due 2026-08-02 and passed unrun.
  - **Why closed rather than re-armed:** the scheduled **Routines** (KR-OPEN WAKE, EOD CONDITIONAL SYNTHESIS) now do this job and do it strictly better — they survive session boundaries, which is the exact property the session-scoped wakes lacked and the reason they died. Re-arming would have duplicated live coverage at duplicate token cost.
  - **The design file is NOT deleted** — `meta/workflow-11-autonomous-day-loop.md` is marked SUPERSEDED in place and retained as the record of what was tried and how it failed. What the item cost while dead: 33 days of session-start briefing attention as a live P1.
  - **Generalizable:** an expiry-dated mechanism whose expiry nobody watches does not announce its own death. It went on being read as live infrastructure for a month after it stopped existing. Companion to the same week's finding that a detector's silence is not evidence of absence.
  - Linked: `meta/workflow-11-autonomous-day-loop.md` (SUPERSEDED), `meta/day-state.md`
...
- [ ] **P2 / process / 2026-08-05** [INFRA, OPT, recurring, weekly] — 🟢 COMPETITIVE-PRODUCT SURVEILLANCE — **CADENCE DECIDED 2026-08-04: WEEKLY** (operator). Was user-gated 13 days. PASS #1 EXECUTED 2026-07-22 (`signals/cross-source-log/2026-07-22-wed-competitive-surveillance-pass1.md`: SKH Δ LOWER / SUMCO watch-firmed / MURATA nil). **Next pass 2026-08-05, then every Wednesday.**
  - Origin: harness-optimization audit 2026-06-26 TIER 3; MRVL Trainium 3 loss + SNDK MU 245TB ION both surfaced POST-HOC via the monthly H2 bear-case — i.e. we have already paid twice for the slower cadence.
  - Scope: per held name, scan competitor-product announcements (1-2 hops to direct substitutes); output per-name competitive-displacement Δ vs prior week.
  - 🔴 **COST LINE CORRECTED 2026-08-04 — the old framing is why this sat for two weeks.** It read *"4× cost vs monthly"*. That is 4 **passes**, not 4× **spend**, and the item's own figures say the spend is a wash: 50-80k/week × 4.345 weeks/month = **217-348k/month, midpoint ~282k**, against the current monthly H2 bear-case at **~280k** → **ratio 1.01×**. Same monthly spend, detection 3-4 weeks earlier. **A decision was gated for two weeks on a cost objection its own numbers never supported** — the arithmetic was sitting in the item the whole time and nobody did it. This is the same instrument-validity failure class as everything else this week: a real quantity (cost) measured with the wrong instrument (passes, not spend).
  - **Blind-check (#51):** distinguishes "a competitor is displacing a held name" from "a competitor announced something" · reads on product announcements within 1-2 hops of a held name's socket · **goes blind if** displacement arrives through a channel that is not an announcement — a design win disclosed only at the customer's print (which is exactly how the MRVL Trainium 3 loss surfaced), a quiet qualification, or a price move. Weekly cadence shortens the lag on the announcement channel and does nothing at all for the others.
  - Kill criterion: if 8 consecutive weekly passes produce zero Δ that the following monthly H2 pass would not have caught, revert to monthly and say so.
...
- [ ] **P1 / prediction / 2026-08-24** [CAL, INDP] — WAKE-CALL STUB-ROW CONVENTION — compliance watch (the forced binary was DECIDED 2026-08-04, branch (a))
  - **Operator decision 2026-08-04: branch (a) — fix the plumbing, not the target.** Diagnosis accepted: this was a RECORDING gap, not a volume gap. Directional calls with resolution dates are made in every wake (H1/H2/H3 reweights, hypothesis tests, vendor-defect predictions) and were never written anywhere gradeable, so they could not count. Branch (b) (re-base 150→~45) rejected: it would have made an honest number out of an unmeasured process.
  - **SHIPPED same day:** `predictions/grading-log.md` § "Wake-call registrations `[WAKE]`" — convention + the first registration. Any wake that states a dated directional call books a one-line stub AT BOOKING TIME with its P-provenance. `[WAKE]` tag exists so the quota can be read BOTH ways — with and without wake calls — which is the guard against the branch-(a) risk the operator was shown (the count inflating with low-stakes calls until 150 means nothing).
  - Scope of THIS item: check #4 and after do not re-measure the gap — they measure **compliance with the convention**. Metric: wakes that stated a dated call ÷ wakes that booked a stub row. Target ≥80% by 2026-08-24.
  - **Blind-check (#51):** distinguishes "the harness is making gradeable calls" from "the harness is making calls nobody records" · reads on the stub-row count vs the dated-call count in wake artifacts · **goes blind if** the stub row becomes the point — i.e. rows get booked that no wake actually reasoned to, or calls get phrased vaguely enough to dodge the "dated" test. Neither shows up in the ratio, which is why the `[WAKE]`-excluded count is reported alongside it and not instead of it.
...
- [ ] **P1 / harness / 2026-08-11** [INFRA, CAL] — 🔧 **G-19 CASCADE-HOOK CHANGESET-WINDOW** — split out of the six-headed P0 on 2026-08-04. `cascade-enforcement-hook.py` computes its changeset against the wrong reference, so a synthesis artifact committed across two Stops can pass the cascade check without every named ticker being back-referenced. K3 refuted the masking rationale that had been used to defer it. Needs a stored-ref design (diff vs last-Stop ref, persisted between fires).
  - **Why this one was rescued and the other five downgraded:** it is the only sub-item that is a live hole in an enforcement hook rather than an inventory or triage task. Rule #19: this touches a LIVE enforcement path → spec first, review-gate, no solo ship.
  - **Blind-check (#51):** distinguishes "the cascade completed" from "the cascade completed inside one Stop window" · reads on the diff between the stored last-Stop ref and HEAD · **goes blind if** the ref is never stored (first fire in a session, or a crashed prior fire) — on which the check silently degrades to today's behaviour rather than failing loudly. The degradation path must be logged, or the fix is invisible when it isn't working.
  - Linked: `meta/hooks/cascade-enforcement-hook.py`, `meta/redteam/2026-07-23-fable5-harness-audit-P0-hardening.md`

- [ ] **P3 / harness / 2026-09-01** [INFRA] — HARNESS AUDIT RESIDUE — triage pass over the ~150 MEDIUM/LOW findings from the 07-23 Fable-5 extract, plus: stop-hook-git-check 8-exit-path inventory; exit-1-outside-repo FP; `test_framework_codifications` 198/200 pre-existing RED (two stale `todo.md` content-couplings — re-scope the test to durable anchors rather than to todo text). **Deliberately P3: this is inventory work, and calling it P0 for eleven days did not make any of it happen.**
  - **Carved out and NOT downgraded** — K3 Q5 blind-spot (3): the blanket `stop_hook_active` guard produces an enforcement-free Stop after every block. That is a LIVE-enforcement defect, Rule #19 review-gated, and it belongs with G-19 above, not in a triage pile. Tracked there.
  - Linked: `meta/redteam/2026-07-23-fable5-harness-audit-P0-hardening.md`

- [x] **P0 / harness / 2026-07-24 → SPLIT 2026-08-04** [INFRA, CAL, DUE] — DEEP-DIVE DEFERRALS + K3 REWORK. **Closed as a single item because it was never a single item.** Six unrelated sub-items of wildly different importance under one P0 tag; 11 days overdue; ranked below 30 other items on two consecutive forced-ranking runs. **The ranker was right and the tag was wrong: a P0 that is six things is a folder, not a priority** — nothing can be "done", so nothing gets started. Split into the two items above (G-19 → P1 dated; residue → P3). Original body retained below for audit. (✅ executed 2026-07-23 by the Fable-5 audit instance, receipts at `meta/redteam/2026-07-23-fable5-harness-audit-P0-hardening.md`, commits 4b26e72 + e477487: all four 🔴 now-or-24 items (a)-(d); G-28 numerator reclassification — **MATERIAL: flips the 08-06 read from FLAT/RISING-retire to FALLING-keep**; e04eaef ref-order adjudicated origin/main-first; redirect over-block FP killed with TPs pinned; probe-pollution channel closed via LLMNA_PROBE tagging). **REMAINING:** (1) G-19 cascade-hook changeset-window (diff vs last-Stop ref — real hole, K3 refuted the masking rationale; needs stored-ref design); (2) stop-hook-git-check 8-exit-path inventory; (3) K3 Q5 blind-spots: blanket stop_hook_active guard = enforcement-free Stop after every block (LIVE-enforcement change → Rule #19 review-gated, spec-then-review, no solo ship), stdin-schema-drift invisible-death class, 337-agent journal receipt gap; (4) exit-1-outside-repo FP; (5) ~150 MEDIUM/LOW from the extract (triage pass); (6) NEW 07-23: test_framework_codifications 198/200 pre-existing RED — two stale todo.md content-couplings, re-scope test to durable anchors. **CERTIFIED (no rework):** f19663d session-start 7→83 (K3 clean on real corpus).
...
- [ ] **P2 / harness / 2026-09-03** [INFRA, CAL, DUE] — **OPTION-B SCORE BLIND-SPOT COUNT.** Two structural blind spots were found in the forced-ranking score within 24h of ship (BLOCKED-ON-OPERATOR missing entirely; `--revive` inserting outside the Open section). **Pre-registered: if a THIRD structural blind spot appears by 2026-09-03, the score is under-specified for this population — replace it with a plain operator-facing "which 30?" prompt rather than patching again.** Patching a score that keeps proving blind is how a decorative mechanism survives.
  - Linked: `meta/backlog-forced-ranking-spec.md` §6
