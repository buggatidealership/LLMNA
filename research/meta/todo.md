...
- [ ] **P0 / research / 2026-07-29** [INDP, POS, DUE] — F1 CASCADE: fix the +1-day June-crash shift (52 lines, 7 theses) once the audit branch addendum lands — **the verify-or-kill is substantially RESOLVED 2026-07-28**
  - Status: **F1 CONFIRMED** — tape independently verified here via EODHD (third vendor), every figure exact (`meta/redteam/2026-07-28-audit-second-commission-DELIVERY-reception-and-independent-tape-verification.md` §1). **USDKRW basis RESOLVED** (Seoul close flat; addendum #10 declared-cut rule live). **KIOXIA fabricated-citation instance CONFIRMED in addition to HYNIX** (two theses, not one). Still open from the original list: claim-4 sharpening ($90.47 = TradingEconomics continuation value) and the cascade-enforcement 0-of-30 instance — both LOW, demoted into this item.
  - Scope: apply the 52-line enumeration by class (A crash→06-23 Tue; B record-high→06-22 Mon 9,114.55; C US-session lines→Tue 06-23 US session AFTER KR close; D recovery→+3.26%; E one KOSPI CB + one KOSDAQ CB + sidecars; F June-8 attribution→06-23 intraday CB trigger). **Held names first: MURATA and SUMCO.** Conflation guard: 06-23 (CB#4) and 06-26 (CB#5, −5.81%) are SEPARATE events with +5.42% (06-25) between. Rule #10 cascade in the same commit.
  - Linked: `origin/claude/verify-investment-os-claims-5u2qb7` (report + pending addendum), reception artifact above
  - Origin: the audit report is NOT on this filesystem; only a relayed summary exists. Same circularity rule the commission imposed on the auditor now applies to the auditor's output.
  - Scope: (1) **F1 — KR/JP crash misdated by one day in 13 places incl. 4 held-name theses** (highest priority; if true it inverts a causal story); (2) **claim 6 — USDKRW −1.09% is the 24h/UTC bar, Seoul close flat** (would be meta-class instance 5); (3) claim 4 — is $90.47 a TradingEconomics continuation value; (4) the 28-fabricated-citation batch + 64-89%-dead-link artifacts; (5) cascade-enforcement 0-of-30 instance. Nothing enters the corpus as established until re-derived.
  - Linked: reception artifact §2 table
