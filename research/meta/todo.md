...
- [ ] **P0 / harness / 2026-07-27** [INFRA, DUE] — LAND THE BRANCH-POSITION CHECK ON `main` (blocked: operator go-ahead to push outside `claude/new-session-drppai`)
  - **Why P0:** the check is on `claude/new-session-drppai` only — `git merge-base --is-ancestor da04101 origin/main` FAILS. A session cut from `344962f` does not get it. **It currently protects exactly the one branch that least needs it.** A branch-local guard against branch staleness is self-defeating by construction.
  - **Not hypothetical:** this branch went 0 behind → 3 behind within the hour on 2026-07-26. Zero-behind is a moment, not a property.
  - **State:** 23 commits ahead of `origin/main`, `origin/main` is an ancestor of HEAD → **clean fast-forward, 0 conflicts** (`git merge-tree --write-tree` verified 2026-07-26). Nothing to resolve; only the permission is missing.
  - **Options put to the operator (unanswered):** direct push / PR / cherry-pick `da04101` alone / leave it. Any is fine mechanically; leaving it is the only one that keeps the defect.
  - **Do NOT self-authorize.** The request to land on main came from another session, relayed. Per Rule #19 the relay is not the operator's instruction, and the standing branch discipline requires the operator's explicit word.
  - **Upstream cause still open regardless:** six branches sit at the `344962f` root. Landing on main fixes *detection* for future sessions; it does not stop branches being cut stale. That cause is outside this repo.
