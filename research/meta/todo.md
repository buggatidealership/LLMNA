...
- [ ] **P1 / research / 2026-06-30** [INDP, AF, POS, BOT] — Build the END-DEMAND-DURABILITY / monetization model (harness framework gap, KJ Leg B 2026-06-30 absence-question). The harness has a SUPPLY model + a FUNDING model (BIS credit node) but NO model for whether AI end-demand is *durable* — the OpenAI-leak ($1.6/$1 burn) reframes the risk as supercycle DURATION, not existence. Co-locate with the AI-funding-shock scenario. **2026-07-01 counter-evidence: BOJ Tankan 8-yr high (large-mfg DI +22, capex +6.8%) explicitly credits AI/semi demand + pricing pass-through = mild DISCONFIRM of U8/F13 near-term (no demand-destruction reaching component/capex layer yet).**
...
- [ ] **P1 / research / 2026-08-15** [INDP, AF, POS, CAL] — BE (Bloom Energy) gas-pipeline TRIPWIRE — load-bearing time-to-power falsifier hard checkpoint
  - Origin: 2026-07-01 BE+ALAB initiation deep-dives (4 Opus 4.8). BE's "weeks-not-years" time-to-power edge for a marquee greenfield build hinges on a ~24-inch gas pipeline reaching in-service ~2026-08-15. If the pipeline slips or gas-supply is the real gating item (not the fuel cells), the speed-advantage partly transfers from a grid-interconnection queue to a gas-pipeline queue — the core BE bull mechanism weakens. This is the single cheapest near-term binary test of the thesis; entry sizing is staged AROUND it (partial now, add after this resolves).
  - Scope: (a) Verify whether the ~Aug-15 pipeline reached in-service on schedule + whether the marquee build energizes at full output; (b) if it slips materially or gas-supply is confirmed as the bottleneck → the "weeks not years" claim for large greenfield builds is falsified/weakened → re-examine BE bull case + hold/trim on any established position; (c) if it lands on schedule → thesis speed-edge CONFIRMED → clears the staged remainder of the BE entry
  - Falsifier: pipeline in-service by ~Aug-15 (± reasonable slip) with the build energizing = thesis CONFIRMED; multi-quarter slip OR gas-supply confirmed as gating = "weeks not years" edge partly transfers to gas queue = BE bull mechanism weakened
  - Also-watch (same window): BE Q2 2026 print (~Jul-30) — gross-margin trajectory (30% Q1 base) + backlog firm-vs-framework split; ALAB Q2 FY2026 (~Aug-11) — Scorpio ramp toward #1-product-line guide
  - Linked: `companies/BE/thesis.md` (INITIATION DEEP-DIVE 2026-07-01), `signals/cross-source-log/2026-07-01-BE-ALAB-initiation-deepdives-4agent.md`, `portfolio/changes.md` (2026-07-01 reallocation intent)
