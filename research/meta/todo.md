...
- [ ] **P1 / harness / 2026-07-24** [INFRA, CAL, DUE] — DEEP-DIVE DEFERRALS (self-patch 2026-07-21 shipped 5 of 8 HIGH probe-gated — see `meta/redteam/2026-07-21-harness-deep-dive-adjudication.md`; these 3 HIGH + tail remain, review-grade): (1) structural-output-metric NUMERATOR reclassification G-28 (semantic change to a pre-registered experiment — MUST land before 08-06; design in the RAW-journal-extract.json); (2) session-prime-cascade HEAD-only changeset window G-19 (diff-against-last-Stop-ref); (3) stop-hook-git-check doc gap → add to CLAUDE.md hook list; (4) NEW FPs found while patching: redirect-guard over-block (anchor protected-path to redirect TARGET), hooks exit-1-outside-repo (fail-open census); (5) ~150 MEDIUM/LOW findings batched from the extract
