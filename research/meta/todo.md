...
- [ ] **P0 / harness / 2026-07-24** [INFRA, CAL, DUE] — DEEP-DIVE DEFERRALS + **K3 CROSS-FAMILY REVIEW REWORK (2026-07-22, ALL flagship claims independently reproduced — `meta/redteam/2026-07-22-K3-deep-dive-review-adjudication.md`)**. **NOW-OR-24 hardening (K3's external payloads = independent probe oracle; user timing decision pending):** (a) 🔴 git-guard `commit -n` bypass — MY 07-21 adjacency-tightening regressed it; `-m "x" -n` → exit 0; catch `-n` anywhere-after-commit; (b) 🔴 git-guard rm boundary — `rm -rf ~/LLMNA/` (trailing slash) + `~/` + `$HOME/LLMNA` all exit 0; REPO_TOKENS the tilde/HOME/slash forms; (c) 🔴 structural-output PI matcher case-brittle (line 171 no re.IGNORECASE) → G-27 reopens on lowercase; add IGNORECASE + 6 casing fixtures; (d) 🔴 test_structural_output_tier_gate fixture RED (case 4 born-red + hardcoded /home/user/LLMNA path) → CLAUDE_PROJECT_DIR-relative + resolve case-4 expectation. **BLOCKING (pre-08-06):** (1) G-28 structural-output-metric NUMERATOR reclassification — RE-GRADED BLOCKING by K3 under our 07-20 precedent, still accruing (2 new tier-fires 07-21); b6ad6d6 size-gate reorder is an accelerant + probe-pollution = 2nd contamination channel; (2) e04eaef ref-order local-main-first vs design-130 origin/main-first — swings week rate 0.196→0.452, adjudicate design intent before 08-06. **Other:** (3) G-19 changeset-window — K3 REFUTED the "masked by one-commit-per-Stop" rationale (main session did 6 commits/10min 07-21); real hole; (4) stop-hook-git-check doc "gap" = NO-OP as scoped (bullet exists CLAUDE.md:154 since May) — real gap is the hook's 8 exit paths never inventoried; (5) NEW FPs (redirect over-block, exit-1-outside-repo); (6) K3 Q5 blind-spots: enforcement-free Stop after every block (blanket stop_hook_active guard incl. commit+push gate), stdin-schema-drift as invisible-death class (reproduced on myself this session), 337-agent journal receipt gap; (7) ~150 MEDIUM/LOW from the extract. **CERTIFIED (no rework):** f19663d session-start 7→83 (K3 clean on real corpus).
...
- [ ] **P2 / harness / 2026-07-24** [INFRA, DUE] — fire-logging as HOUSE STANDARD + B47 EFFICIENCY-CLAIM HOOK DECISION: shared log_fire helper module + migrate remaining Stop hooks
  ...
  - Scope: meta/hooks/hook_fire_log.py shared helper (house format, repr-escape, fail-open); migrate remaining Stop hooks opportunistically at the 07-24 monthly audit; verify no double-fire/format break (structural-output-metric.py filters by hook name — format is proven multi-tenant). ALSO (K3 r2): fire-log ROTATION/cap policy (append-only with no rotation — the session-prime MAX_INJECT cap lesson applies to logs too) + monthly clock-source cross-check (container clock vs git-remote timestamps; the TODAY header inherits the container clock unverified). ALSO (prose-sweep #1 items 3+6, bound 2026-07-20): wire the Test-D prose-deadline extractor into session-start-hook.py as a deterministic pass + add parser coverage tests to meta/tests/ (real todo.md + grading-log.md fixtures, fail on unmatched live rows). ALSO (2026-07-20, B47 N=2): DECIDE + design the B47 efficiency-claim Stop hook (trigger: X-times efficiency/power/cost-per-unit claim in analytical output WITHOUT a U8/B47/Jevons/demand-destruction reference; both B47 instances were externally caught, so the design goes through review-grade process — spec then K3/fresh-session review — NOT solo ship; interim = CLAUDE.md INGEST step 4b, live 2026-07-20).
