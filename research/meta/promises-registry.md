...
- [ ] DUE:2026-07-24 | WHAT: evaluate a reviewed repo-side commit+push backstop (content-scoped guard) to cover the stop-hook-git-check nap window + headless-absence — double-fire risk needs review before shipping | SOURCE: meta/guard-exit-path-inventory.md §B
- [ ] DUE:2026-08-01 | WHAT: propagate the content-scoped recursion guard from receipts-hook to the integrity-critical Stop hooks (anti-fabrication first), reviewed per-hook | SOURCE: meta/guard-exit-path-inventory.md §C
