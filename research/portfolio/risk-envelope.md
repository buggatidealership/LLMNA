...
**Baseline anchor:** 2026-07-05, account €95.406,73 (screenshot T1, `holdings.md` canonical). Later deposits/withdrawals adjust the base (money-weighted).
**Baseline benchmark closes FROZEN 2026-07-06 (US closed Fri Jul-3 → baseline = Jul-2, 2026 official closes, per `signals/cross-source-log/2026-07-06-kr-jp-morning-scan-prelim-eve.md`, T1/T2):** Nasdaq-100 29,329.21 / S&P 500 7,483.24 / NVDA 194.83 / AMD 517.82 / AVGO 360.45 / TSM 434.16. Note: Jul-2 was a semis-selloff day (SOX −6.7%) — a slightly LOW bar for the basket legs, i.e. conservative against us; recorded, not adjusted.
...
## v2 DERIVATION FROM THE PURPOSE ANSWER (2026-07-12 — anchored to `meta/user-source-profile.md` §purpose; supersedes assumption-based framing; Phase-1 cap still USER-GATED)
- **Context reframe:** the book = 24.6% of ~€380k investable (computed); full-book loss = user-stated sleepable (€280k ≈ 3yr runway floor outside). The envelope's job is therefore NOT existential protection — it is (a) protecting the €280k floor absolutely (NO leverage/margin/instruments that can exceed the book — standing rule), (b) drawdown-QUALITY inside the book (his reflective self is loss-regret-dominant: gates firm), (c) keeping the reserve-tranche bar honest (~€100k comfortable ceiling; up to ~€200k ONLY at near-deterministic conviction — define operationally: pre-registered P≥85 my-model + Rule #17 ensemble 5/5 agreement + user sign-off; anything less does NOT qualify for reserve deployment).
- **Phase-1 single-position cap ≤15% — RE-PROPOSED on the new basis:** with full-book loss survivable, the cap serves compounding-quality (avoiding single-name path destruction), not survival. SUMCO at ~17.9% remains the live case. DECISION STILL YOURS — but now the question is precise: "accept single-name concentration risk to path-quality, yes/no?"
- **North Star instrumented:** FI ≈ €4.0-5.0M at €200k/yr passive (computed, 4-5% yield assumption) — added as the harness's terminal metric; progress = total investable trajectory, reviewed at monthly audits. No date pressure per user; grade the PATH (drawdowns, calibration), not calendar speed.

## v3 — THE CAPITAL LADDER (2026-07-12, from principal-question #6; my recommendation section = Rule #18 dissent stated)
| Rung | Book ceiling | Promotion gate (pre-registered — raises happen on GRADED evidence, never on enthusiasm) |
|---|---|---|
| **A (current)** | **€100k** | in force now (book €93,597.97 = headroom exists for staged adds within existing tier rules) |
| B | €140k | 2 consecutive monthly audits with calibration bands in-tolerance + the SKHY gate-vs-override episode RESOLVED and lessoned + one full earnings season (Aug) graded |
| C | €180k (user's stretch size) | rung-B held ≥2 months + a real drawdown experienced and slept through (user-confirmed, not assumed) + North-Star trajectory review |
| Reserve beyond C (to €220k max) | only at the near-deterministic bar (P≥85 pre-registered + ensemble 5/5 + user sign-off) | unchanged from v2 |
| **FLOOR** | **€160k outside the book, inviolable** (user's rational 2yr-runway number) | enthusiasm-case €280k deployment REJECTED as inconsistent: leaves €100k reserve = €60k short of the user's own floor (computed) |
**Rule #18 dissent, explicit:** the strongest case FOR fast escalation is the user's own compounding logic (bigger pool grows faster toward the €4.0-5.0M North Star) — it fails NOW because (a) it contradicts his own floor arithmetic (computed above), (b) the harness has not yet completed ONE full grading season with real money at current size (SKHY is 2 days old; first earnings gauntlet is August), and (c) his reflective self is loss-regret-dominant (§purpose) — the ladder serves that self. Capital promotion is EARNED by the record, not granted by excitement. All transfers = user actions; the harness's role = certifying gates. Phase-1 single-position cap (≤15%) still awaits explicit user yes/no. 🟡


## v4 NOTE (2026-07-12): Phase-1 cap RESOLVED — no user-imposed cap; the binding disciplines re-ranked by the principal's own history
User: no position-count/size preference — outcome only. Cap withdrawn. **What the volunteered financial history re-ranks as the envelope's real protections (per user-source-profile §history):** (1) **anti-panic-exit = Rule #8/B9 — THE highest-value protection** (his Jan-Feb 2026 memory-cohort exit = the empirical validation case); (2) no leverage — inviolable (−€180k realized history); (3) mandate boundaries (no memes, scam-surface minimized); (4) the capital ladder (v3) unchanged — promotion by graded evidence. Concentration handled per-thesis via tier bands + ensemble discipline, not a hard cap. Profile-fluidity rule applies: these settings re-confirmable, not ossified. 🟡

**2026-07-12 external-convergence note (no rule change):** Griffin/Goldman APEX (verified, per `signals/cross-source-log/2026-07-12-sun-eve-griffin-goldman-apex-verification.md`): "know the worst case, know whether you can tolerate it, hold exposures so the extreme outcome stays inside what you can survive" — independently identical to this envelope's capital-ladder logic (€160k inviolable floor = the survivable-loss boundary operationalized). Recorded as convergence, not authority.
