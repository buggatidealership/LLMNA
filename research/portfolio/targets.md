...
**2026-07-02 PM — USER DECISION (Rule #11 gate closed):** *"keep them on the watch list."* All five buy-list names (BE, ALAB, ONTO, AVGO, Sumitomo Bakelite) remain WATCHLIST — no entries executed. The earmarked DeGiro cash (~€19.967) stays uncommitted. Standing re-triggers (unchanged, will re-surface with evidence, decision remains user's): Samsung prelim ~Jul-7 (positioning-vs-fundamentals adjudicator), BE Jul-30 guide print, ALAB Aug-11 print (post-print dip window), ONTO quote-conflict resolution + Aug-06 print, AVGO next-money queue, 4203 exposure dig. Watch status does NOT expire these predictions — all resolve and get graded regardless.

---
## 🚨 2026-07-05 — ALL PRIOR TARGETS SUSPENDED (fresh-start baseline)
Per `portfolio/holdings.md` 2026-07-05 canonical: book = MURATA 20,6% + SUMCO 17,9% + cash 61,5% (€95.406,73 total). All prior per-name targets and the 2026-07-02 watchlist re-triggers remain REFERENCE ONLY — no sizing proposals of any kind until `portfolio/risk-envelope.md` exists (drawdown tolerance + tax regime pending from user; scale + full-deployability answered 2026-07-05). Targets file gets rebuilt envelope-first, not name-first.
