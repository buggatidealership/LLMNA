...
**🟡 2026-07-02 PM ECTC custom-HBM mechanism — the HBM-attach watch-line now has its MECHANISM (two-sided):** Per `signals/cross-source-log/2026-07-02-ectc-2026-packaging-roundup-semianalysis-ingest.md` (SemiAnalysis ECTC roundup T2 + 1 cross-claim agent). Custom HBM base dies on foundry logic processes (Marvell: host HBM-PHY area ~60% freed; Feynman-custom-HBM cross-claim pending) = BULL for memory CONTENT (base-die value grows, LPDDR-attach adds a conventional-DRAM demand vector on accelerators — consistent with the DDR5-discipline read) but a WATCH for value MIGRATION (base die moves to whoever fabs it — Samsung's own-foundry custom-HBM pitch — and to designers like Marvell; long-term commoditization risk for the standard-JEDEC stack layer). SKH counter: HBM leadership + its own custom-HBM base-die roadmap. HBM4E packaging difficulty (2× interposer layers, +86% power vs HBM3E, Samsung ECTC) = complexity moat SUPPORTS pricing power near-term. **Position implication: ⚪ NO ACTION — mechanism logged under the existing watch-line; triggers unchanged (Anthropic HBM-supplier reporting + any custom-HBM design-loss datapoint); no falsifier touched.**
