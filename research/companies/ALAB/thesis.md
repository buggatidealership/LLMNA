...
**TL;DR:** ALAB is executing near-flawlessly (Q1 FY2026 rev **$308.4M +93% YoY**, GM **76.3%**, GAAP NI $80.3M, EPS $0.44; Q2 guide $355-365M +15-18% QoQ, beat-and-raise intact; ~$1.18B cash, near debt-free — T1 10-Q). **🟢 MATERIAL CORRECTION: the prior "~70% single-customer (AWS)" concentration flag is STALE — the Q1 FY2026 10-Q shows top customer 29%, five customers each ≥12% (29/21/16/12/12%). Concentration has MATERIALLY IMPROVED = the SKIP-SIZE flag's concentration leg is largely RESOLVED.** Scorpio scale-up switching (~15% of FY25 rev) guided to become the **#1 product line by end-FY2026** = the growth-bet converting to revenue. PCIe-6 already >⅓ of revenue.

**The binding constraint is now VALUATION, not concentration:** ~$485, ~$72B cap, **~50-55x fwd P/S / ~125x fwd P/E, +200%/3mo, ~70% ABOVE avg analyst target**; Nasdaq-100 inclusion (2026-06-22) drove mechanical passive buying + the +16% Jun-29 spike (technical, not fundamental); the Q1 four-line beat still SOLD OFF = priced-for-perfection tape. (B45: not reflexively "overvalued" — but entry-timing risk is high; a beat may already be priced.)

**Conviction (my model):** P(bull) ~35% (+80-150%/18-24mo IF Scorpio ramps + diversification holds) · P(base) ~40% (flat-to-+25%, multiple compresses as growth normalizes) · P(bear) ~25% (−45-60% on a de-rate from ~50x fwd-sales + a ramp/concentration shock). Anti-fragility ~3.5/6 (wins the merchant-connectivity-TAM-stays-open scenarios; loses the two high-severity tails: NVIDIA verticalization + a concentration shock). **Not Core-grade — a high-beta bet on merchant connectivity staying open.**

**Competitive:** wins on protocol-agnostic switching (PCIe + NVLink Fusion + UALink + Ethernet under COSMOS) + PCIe-6 volume lead + software lock-in. Threats: NVIDIA verticalization/NVLink + CPO stripping retimer content (2028+, slow); AVGO bundling switch+retimer (margin pressure — Q2 GM guided ~73% on a one-time non-cash warrant); MRVL/Credo overlap.

**Falsifiers (dated):** (1) by Q4-FY2026 (~Feb-2027) top-customer share does NOT stay ≤~30% / no continued diversification → re-concentration; (2) by 2H-FY2026 Scorpio fails to become #1 product line OR X-Series HVM slips into 2027 → the valuation-core ramp breaks; (3) FY2026-exit non-GAAP GM sustainably <~70% ex-warrant on AVGO/MRVL price competition → moat/pricing-power falsified; (4) a design-win loss disclosed to NVIDIA CPO or an AVGO bundle at the top customer.

**Position implication: 🟡 ENTER (initiate ACTIVE) — user-directed €10k; MY RECOMMENDATION = START ~€5-7k now + stage the remainder on a pullback or after the ~2026-08-11 Q2 print (research-verified 2026-07-01, T1/T2) — the concentration fear is RESOLVED (29% not 70%, genuinely de-risking + supports the buy) but ~50-55x fwd-sales + ~70%-above-target + a Nasdaq-100-inclusion technical spike make full €10k at the top of a +200%/3mo run an unfavorable entry; SKIP-SIZE DOWNGRADED to REDUCED-SIZE (valuation-driven, not concentration). Sizing user-gated (Rule #11); holdings.md updates on post-trade screenshot.**

---

**⚪ 2026-06-30 ALAB × ROBOTICS intersection — NONE (verified; do NOT tag as a robotics name):** Per `signals/cross-source-log/2026-06-30-agentic-discovery-test-time-compute-ALAB-criticality-demand-durability.md` companion check (1 Opus 4.8). No direct intersection: no robotics/edge SKU; no humanoid/robotaxi design win (ABSENT from NVIDIA Jetson Thor partner list); no management "physical AI"/edge TAM language; the robot itself (edge inference on Jetson/Thor-class silicon) contains ZERO ALAB content. ONLY link = INDIRECT/undifferentiated (robotics foundation-model TRAINING runs in datacenters on GPU clusters using ALAB fabric = generic AI-training exposure, not a robotics thesis). 🚩 Rejected an unsourced "ALAB automotive/AV partnership" claim (no primary source — do not propagate). **Position implication: ⚪ NO ACTION — classification clarified — ALAB = datacenter-AI-connectivity ONLY; any robotics upside already captured in generic AI-training-capex exposure; do not create a robotics tag.**

**🟡 2026-06-30 agentic-discovery criticality verification — ALAB is 2nd-order/merchant-broadening, NOT frontier-first-instance (~75%):** Per `signals/cross-source-log/2026-06-30-agentic-discovery-test-time-compute-ALAB-criticality-demand-durability.md` (1 Opus 4.8). The frontier reasoning-discovery breakthroughs (gluon-amplitude physics ~12hr GPT-5.2; Erdős #397/#1196 Tao-verified) are TEST-TIME-COMPUTE-gated = the scale-up-interconnect LAYER is co-1st-order — BUT they ran on **NVIDIA NVLink / Google TPU-ICI (Broadcom), NOT ALAB's merchant fabric.** ALAB's fabric is NOT inside the NVLink72/TPU-ICI domain carrying the KV-cache all-to-all for a frontier query. **ALAB = co-1st-order ONLY within the non-NVIDIA MERCHANT scale-up world** (Scorpio-X ~$2.5B AWS 2026-29; UALink ~$17B TAM 2027-28; PCIe-6 retimers) — the discovery tailwind reaches ALAB via the 2nd-derivative (AWS/AMD scale-up ramps materializing 2026-28), not the frontier-first-instance. **Position implication: 🟡 WATCHLIST — no entry — discovery thesis makes the interconnect-CATEGORY load-bearing but ALAB the merchant-slice 2nd-derivative (needs AWS/AMD ramp to capture it); reinforces ALAB-as-broadening-play (not frontier-gated); reallocation on hold.**

**🟡 2026-06-30 displacement-risk verdict (my-model synthesis on verified substrate) — the ANTI-FRAGILE merchant connectivity holding:** Builds on the same-day ALAB-vs-MRVL artifact. Two-axis convex hull for scale-up interconnect: (1) electrical→optical (copper hits a reach wall as scale-up domains go multi-rack), (2) proprietary(NVLink)→merchant(UALink/PCIe). **ALAB's displacement risk is a SLOW, telegraphed TECHNOLOGY-SUBSTITUTION risk** (its copper core yields to optical as domains scale past the rack, ~2028+) — but it is **protocol-agnostic (wins under NVLink Fusion + UALink + PCIe) AND building optical PCIe = holds a call-option on the very transition that threatens it.** So it wins the present (electrical/AEC, 5+yr runway) AND rides the optical transition = **anti-fragile across both axes (H2 ~30%, but the scenario-robust choice).** vs MRVL = leveraged bet on the optical destination (wins bigger in fewer scenarios). **My-model call: ALAB wins in MORE scenarios; the optical APPROACH wins the end-state architecture but NVIDIA keeps the proprietary crown + the unbypassable rent is memory (HBM), not the contested merchant scale-up layer.** 🔴 Rule #18 bear (~30%): if scale-up scales FASTER than expected (NVL576→pod by 2027-28), the copper wall arrives before ALAB pivots → electrical moat strands, MRVL/Celestial owns the only merchant optical-scale-up stack at the moment it matters. **Position implication: 🟡 WATCHLIST — no entry — confirmed as the anti-fragile/scenario-robust connectivity-innovation expression (wins present + transition optionality); ALAB the cleaner merchant bet vs MRVL's leveraged-call; SKIP-SIZE (>$50B cap + AWS concentration) + reallocation on hold persist.**
...
**📌 [2026-07-02 PM] CPO-TIMING DISPUTE LOGGED (🟡 DIRECTIONAL/DISPUTED — pre-entry watch enrichment):** SemiAnalysis (2026-06-09, named primary per D1 verification, `signals/cross-source-log/2026-07-02-oblivious-layer-program-wave2-3-final-adjudication.md`) reports Nvidia large-scale CPO mass production delayed to 2028-29 on optical-engine yield (~19% system yield at 32 engines/switch) + serviceability concerns — triggered a real optical-stock shakeout. DIRECTLY DISPUTED by Nvidia (Gilad Shainer, Computex: volume shipments start H2 2026). Read for the entry intent: if SemiAnalysis is right, retimer/AEC socket life EXTENDS (supportive, at the margin, of the ALAB position case); if Nvidia is right, the dated optical bear-watch below stands exactly as written. Neither branch changes the entry plan or tranche size — both sides now dated for the T+n re-check. Position implication: NO ACTION — 🟡 — entry intent and tranche unchanged; dispute resolves via H2-26 CPO shipment evidence.
