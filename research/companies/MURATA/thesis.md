...
**🟡 2026-07-23 cross-ref — Renesas trade-secret suit vs Navitas = falsification-monitor haircut (TC-11 N=2):** Per `signals/cross-source-log/2026-07-23-semidoped-quicktakes-7item-newsletter-ingest.md`. Renesas filed a trade-secret lawsuit against Navitas Semiconductor (T3 Semi Doped newsletter relay of Reuters T2 — VERIFY). Relevance to MURATA: the pre-registered falsification monitor on the tray-level MLCC content thesis is Navitas's direct 800V→6V single-stage GaN board scaling to >50% of server trays (Rubin Ultra Kyber H2 2027 window, per `watchlist/candidates.md` + `sector/where-we-are.md`). 2nd order (P~60%, my model): litigation friction/injunction tail-risk on Navitas during its AI-DC GaN ramp marginally LOWERS the probability that the single-stage architecture displaces the multi-stage conversion chain on schedule → the MLCC-content falsifier becomes slightly less likely to fire. Not a thesis driver — a falsifier-probability haircut. **Position implication: 🟡 HOLD Core 20.6% (2026-07-05 canonical snapshot) — no size change — falsification-monitor risk marginally reduced; Jul-31 Q1 FY27 print remains the standing sizing-review trigger per Round 6 flag.**
