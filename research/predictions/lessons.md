...
**L42-b (codified): every percent-change figure entering the corpus carries a BASIS STAMP — `settle` / `intraday` / `extended-hours` — or it is unusable. A number without a basis is not a weak datum; it is not a datum.**
