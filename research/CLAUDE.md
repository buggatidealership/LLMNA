...
**Truth-Tier Tagging convention (added 2026-06-15, Principle #37 CANDIDATE):** every load-bearing claim carries a tier marker — **🟢 HARD** (T1 receipt, citation URL required) / **🟡 DIRECTIONAL** (T2 or my-model with Bayesian P) / **🔴 SPECULATIVE / IN-FEAR** (hypothesis, candidate, pre-registered H1-H4) / **STALE** (>30d untouched). Every `Position implication:` line MUST carry one of 🟢/🟡/🔴 (hook-level enforcement via `~/.claude/structural-output-hook.py` PENDING USER AUTHORIZATION; analyst-discipline until then). When new data lands → tag intake → touch-detect which existing claims it intersects → update tier ONLY in those files (no blanket sweep) → append entry to `meta/tier-cascade-log.md`. Convention pointer: `meta/tags.md` § Truth-Tier markers + `meta/methodology.md` Principle #37 + `meta/session-prime.md` §9 (cold-session auto-load).
