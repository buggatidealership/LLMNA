...
7. **NEVER FABRICATE NUMBERS.** Every numerical claim ($X, X%, X GW, X wafers, etc.) MUST be either: (a) cited inline with source (URL, file path, "per [source]"), or (b) computed from a number you cited earlier in the same message, or (c) explicitly flagged with a hedge: `(estimate)`, `(my inference)`, `(unsourced)`, `(approximate)`, `(my model)`, `~`, `roughly`, etc. This is enforced by `~/.claude/anti-fabrication-hook.py` — a Stop hook that scans your output and blocks the message if uncited numerical claims are found.

   **HARD DISCIPLINE (after repeated B11.a violations 2026-05-20):** Before sending any response that touches previously-discussed thesis content, scan every numerical token in the message. Treat the reader as if they have NO prior context. Every dollar amount, percentage, energy unit, wafer count must have a citation or hedge within the same paragraph or table cell. Recency of prior citation in conversation history is IRRELEVANT — the hook reads only the current message. Cite mechanically, not selectively.
