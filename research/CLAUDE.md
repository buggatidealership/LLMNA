...
- **`research/meta/hooks/session-prime-cascade-hook.py`** — Stop hook (added 2026-07-12, escalation pre-registered in session-prime.md §10 + the 2026-07-12 effectiveness audit, which found Jul-9→12 codification batches shipped without session-prime updates). NARROW trigger: fires only when the changeset ADDS a new codification ID (new `### B-n`/`TC-n`/Principle header, new `**L-n —` lesson, new dated Critical Rule) to a canonical file (biases-watchlist / lessons / methodology / CLAUDE.md / triangulation.md) while `meta/session-prime.md` is absent from the same changeset. N-counter increments and reference edits never fire it. Fail-open on any exception. Falsifier (re-eval 2026-08-12): zero fires in 30 days → refine or retire; false-positive rate >30% → tighten patterns.
