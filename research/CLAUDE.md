...
**Hook activation architecture (Architecture A, 2026-06-26):** all 16 research-OS hooks are registered in **`<repo-root>/.claude/settings.json`** (project-level, version-controlled) with absolute paths to in-repo hook scripts at `/home/user/Health-Calculators/research/meta/hooks/*.py`. This file ships with the repo via `git clone`, so every fresh container has hooks active from turn 1 — zero laptop dependency, zero `install.sh` dependency, zero `~/.claude/` dependency. `~/.claude/settings.json` is intentionally kept empty to prevent merge duplication with system-managed `launcher-settings.json` hooks. Full rationale + verification protocol + migration history at `research/meta/hooks/DURABLE-ACTIVATION.md`. The hook paths below still read `~/.claude/...` for historical reference — the actual live execution paths are now the in-repo locations.
